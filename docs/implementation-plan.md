# Ultravox v1 Implementation Plan
...
**Architecture:** Standalone single-package Tauri 2 project at `/Users/benjaminkurtz/Documents/localcoding/ultravox/`. React renders inside a frameless transparent pill window for recording and a standard settings window. Rust main process owns global hotkey, paste-to-frontmost-app, frontmost-app detection, tray, and window management. The existing CF Voice Worker (in the sibling bka2brain project) is reused unchanged via HTTP — Ultravox does **not** import bka2brain code at build time.

**Tech Stack:** Tauri 2, React 19 + TypeScript, Vite, Tailwind v4 (`@tailwindcss/vite`), Rust (with `enigo`, `tauri-plugin-global-shortcut`, `tauri-plugin-store`, `tauri-plugin-stronghold`, `tauri-plugin-updater`, `objc2`/`windows-rs` for OS APIs), CF Voice Worker (existing, unchanged).

**Working name:** `ultravox`. Real name + bundle ID locked before first signed distributable build.

**Companion docs:** [research.md](./research.md), [design.md](./design.md)

---

> ## Plan adjustment notice — small monorepo + shared design system + theming
>
> **This notice supersedes the original Phase 1 below and adds cross-cutting
> requirements to several later phases.** The full refinement spec lives at
> `~/.claude/plans/read-claude-md-and-generic-muffin.md` (approved 2026-05-05).
> Summary of what changed since the document below was first written:
>
> ### Repo layout — small `pnpm` monorepo (not a single package, not a bka2brain monorepo conversion)
>
> Ultravox stays at `/Users/benjaminkurtz/Documents/localcoding/ultravox/` and bka2brain stays untouched. Inside this repo:
>
> ```
> ultravox/
...
> ├── apps/
> │   └── ultravox/                   ← the Tauri app (was the planned single-package layout)
> ├── packages/
> │   └── design-system/              ← tokens.css + fonts + theme runtime
> └── source-material/                ← unchanged copy from bka2brain
> ```
>
> The `packages/design-system` package exists today specifically so the future
> Ultravox marketing website can consume the **same** tokens, fonts, and theme
> APIs without a copy-paste — see refinement spec for rationale. It is **not**
> published to npm in v1; consumed only via the workspace symlink.
>
> No `apps/bka2brain/` and no `packages/voice-core/` in v1. Voice components
> are still ported directly from `source-material/` into `apps/ultravox/src/`
> as the implementation needs them.
>
> ### Bka2brain design system ported wholesale
>
> - `packages/design-system/src/tokens.css` is a verbatim copy of
>   `source-material/styles/tokens.css` — three themes (light, dark-ocean,
>   dark-night), 18 typography tokens, ink/surface scales, motion, dark-mode
>   utility overrides. Do **not** prune in v1.
> - Bundled fonts: **DM Sans** (400/500/600) + **Cormorant Garamond** (400 +
>   400 italic) as `.woff2` under `packages/design-system/assets/fonts/`.
>   Kalam (handwriting) is intentionally skipped in v1.
> - Tailwind v4 setup happens in **the app**, not the package:
>   `pnpm add -D tailwindcss @tailwindcss/vite` in `apps/ultravox`, register
>   `tailwindcss()` in `vite.config.ts`, and have `apps/ultravox/src/main.tsx`
>   import in this exact order:
>   ```ts
>   import "@ultravox/design-system/fonts.css";
>   import "@ultravox/design-system/tokens.css";
>   ```
> - **Use composite typography utilities throughout the UI.** Settings
>   headings → `typography-h3`/`typography-h4`. Sidebar labels →
>   `typography-menu-text` or `settings-label`. Body copy → `typography-body`
>   / `typography-body-narrative`. Section labels → `typography-label`.
>   Pill mode label → `typography-menu-text-on-dark`. Hint chips →
>   `typography-hint` size + `text-secondary-on-dark`. Hardcoded RGB stays
>   banned; reach for raw `text-color-*` / `bg-color-*` only when no
>   composite utility fits.
>
> ### Theming subsystem (Auto / Light / Dark Ocean / Dark Night)
>
> - Theme runtime lives in `packages/design-system/src/theme.ts` and is
>   **runtime-agnostic** — no Tauri imports. Exports
>   `applyTheme(choice)`, `subscribeSystemAppearance(cb)`, and
>   `createThemeController(storage)` against a small `ThemeStorage` interface.
> - `apps/ultravox/src/lib/theme-storage.ts` adapts `tauri-plugin-store` to
>   the `ThemeStorage` interface. `apps/ultravox/src/lib/theme-bridge.ts`
>   broadcasts and listens for the `theme:changed` Tauri event so the pill
>   window, mode overlay, and settings window (separate WebViews) all repaint
>   together without a relaunch.
> - Each Tauri window's React entry calls
>   `applyTheme(await controller.getStoredTheme())` **before** rendering, then
>   subscribes to both `subscribeSystemAppearance` and `theme:changed`.
> - Default user choice: `'auto'`. Default dark resolution under `'auto'`:
>   `'dark-ocean'` (matches the navy floating-pill chrome).
> - Theme picker UI lives **inside the Configuration panel** as a 4-option
>   segmented control with bg+accent swatches. No new sidebar entry.
> - The pill chrome stays dark navy in all themes for legibility on any
>   desktop; inner content uses `*-on-dark` utilities, which already track
>   theme via the dark-mode utility overrides in `tokens.css`. The
>   `⌥⇧K` mode overlay consumes `--quick-menu-bg/border/fg`, which auto-flip
>   in dark.
>
> ### Phase-by-phase carryover (apply when you reach each phase)
>
> | Original phase | Adjustment |
> |---|---|
> | **Phase 1** | Replace entirely. Initialize the workspace described above (`pnpm init -w`, write `pnpm-workspace.yaml`, create `tsconfig.base.json`), create `packages/design-system/` with the layout above (already partially done as part of the refinement landing — verify before redoing). Do not extract `voice-core` and do not touch bka2brain. |
> | **Phase 2** | Tauri scaffold goes into `apps/ultravox/`, not the repo root. Add `"@ultravox/design-system": "workspace:*"` to its `package.json`. CSS imports order in `main.tsx`: fonts → tokens. |
> | **Phase 3** | Components ported from `source-material/` must use composite typography utilities and on-dark variants for content rendered on the pill chrome. |
> | **Phase 5–6** (window plumbing + Settings shell) | Add `apps/ultravox/src/lib/theme-storage.ts` and `theme-bridge.ts`. Each window calls `applyTheme(await getStoredTheme())` pre-render. |
> | **Phase 12** (Settings shell + 6 panels) | Configuration panel gets a Theme group at the top: segmented control Auto / Light / Dark Ocean / Dark Night with swatch previews. On change → `setStoredTheme` → `applyTheme` → emit `theme:changed`. Use `typography-h3`/`label`/`body`/`menu-text`/`settings-label` over raw `text-color-*` throughout all panels. |
> | **Phase 13** (Onboarding) | Hero copy uses `typography-h2` (Cormorant) + `typography-body-narrative` for editorial tone. |
> | **Phase 17** (cross-app smoke matrix) | Add row: pill + settings + mode overlay render correctly with system in Light, system in Dark, and after manual override to Light / Dark Ocean / Dark Night. |
> | **Phase 14–17** (verification) | Verify: theme persists across relaunch; manual override repaints all three windows without relaunch; OS appearance flip while `'auto'` is selected updates all three windows within ~100 ms; reduced-motion users get no quick-menu / voice-line / toast animations. |
>
> The reference code at `source-material/` is copied — not symlinked — from
> bka2brain. Port files from there into `apps/ultravox/src/` as needed and
> convert `.jsx` → `.tsx` along the way.
...
- [ ] Branch created: `git checkout -b feat/ultravox-v1`
...
```json
{
  ...
  "scripts": {
    ...
    "dev:ultravox": "pnpm --filter ultravox tauri dev",
    "build:bka2brain": "pnpm --filter bka2brain build",
    "build:ultravox": "pnpm --filter ultravox tauri build",
...
**Goal:** Create `apps/ultravox` with Tauri 2 + React + TypeScript + Vite. App opens a window that says "Ultravox" with a button. No actual functionality yet — just verifying the toolchain.

**Outcome at end of phase:** `pnpm dev:ultravox` opens a Tauri dev window with React inside.

### Task 2.1 — Create Tauri app via CLI

**Files:** All files under `apps/ultravox/` (created by CLI)

- [ ] **Step 2.1.1: Run create-tauri-app**

```bash
cd apps
pnpm create tauri-app ultravox --template react-ts --manager pnpm --identifier com.ultravox.dev
cd ..
```

Expected: `apps/ultravox/` is created with `src/`, `src-tauri/`, `package.json`, `tauri.conf.json`.

- [ ] **Step 2.1.2: Set the package name to match workspace convention**

In `apps/ultravox/package.json`, set `"name": "ultravox"` and add voice-core dep:
...
```bash
git add apps/ultravox
git commit -m "feat(ultravox): scaffold Tauri 2 + React + TS"
```

### Task 2.2 — Configure tauri.conf.json for app identity

**Files:** `apps/ultravox/src-tauri/tauri.conf.json`
...
```json
{
  ...
  "productName": "Ultravox",
  "version": "0.1.0",
  "identifier": "com.ultravox.dev",
  "build": {
...
Expected: window title is "Ultravox", size 960x640.

- [ ] **Step 2.2.4: Commit**

```bash
git add apps/ultravox/src-tauri/tauri.conf.json apps/ultravox/src-tauri/Cargo.toml
git commit -m "feat(ultravox): set app identity"
```
...
**Goal:** Centralize the app name, bundle id, domain, and other branding strings in one place. Wire voice-core's `tokens.css` into the Ultravox UI so future components inherit bka2brain's visual language.
...
export const BRANDING = {
  ...
  domain: 'ultravox.local',
  supportEmail: 'support@ultravox.local',
  marketingUrl: 'https://ultravox.local',
} as const
...
### Task 3.2 — Wire tokens.css into ultravox

**Files:**
- Modify: `apps/ultravox/src/main.tsx`
- Modify: `apps/ultravox/src/App.tsx`
...
- [ ] **Step 3.2.3: Run ultravox**

```bash
pnpm dev:ultravox
```

Expected: window shows "Ultravox" in dark navy on cream background.

- [ ] **Step 3.2.4: Commit**

```bash
git add apps/ultravox/src
git commit -m "feat(ultravox): wire tokens.css and branding"
```
...
**Goal:** A `useRecorder` hook in voice-core that captures mic audio and a `transcribe` function that POSTs to the CF Voice Worker. Ultravox wires them together with a temporary in-window button.
...
### Task 4.4 — Wire dev token endpoint into ultravox

For now, ultravox reuses bka2brain's local token endpoint (`http://localhost:8787/api/voice/token`) during dev. v1.5 introduces a hosted endpoint.

**Files:** `apps/ultravox/src/lib/tauri-bridge.ts` (just constants for now)
...
export const TOKEN_ENDPOINT = import.meta.env.DEV
  ...
  : 'https://api.ultravox.local/voice/token'
```
...
```bash
git add apps/ultravox/src
git commit -m "feat(ultravox): wire useRecorder + transcribe through to dev UI"
```
...
**Files:** `apps/ultravox/src-tauri/Cargo.toml`
...
```bash
git add apps/ultravox/src-tauri/Cargo.toml apps/ultravox/src-tauri/Cargo.lock
git commit -m "feat(ultravox): add enigo + clipboard plugin deps"
```

### Task 5.2 — Implement paste.rs

**Files:**
- Create: `apps/ultravox/src-tauri/src/paste.rs`
- Modify: `apps/ultravox/src-tauri/src/lib.rs`
...
`apps/ultravox/src-tauri/capabilities/default.json`:

```json
{
  "$schema": "../gen/schemas/desktop-schema.json",
  "identifier": "default",
  "description": "Default permissions for Ultravox",
...
```bash
git add apps/ultravox/src-tauri/src/paste.rs apps/ultravox/src-tauri/src/lib.rs apps/ultravox/src-tauri/capabilities/default.json
git commit -m "feat(ultravox): add paste_to_frontmost Rust command"
```

### Task 5.3 — Wire paste into the React test flow

**Files:** Modify `apps/ultravox/src/App.tsx`, `apps/ultravox/src/lib/tauri-bridge.ts`
...
Open TextEdit. Click into the document body. Then in Ultravox dev window, click "Test record", speak "this is a test", click "Stop". Expected: text appears in TextEdit.
...
```bash
git add apps/ultravox/src
git commit -m "feat(ultravox): paste transcribed text into frontmost app"
```
...
**Goal:** Tauri command `get_frontmost_bundle_id() -> String` returns the bundle id of whichever app is currently in front of Ultravox.
...
**Files:**
- Create: `apps/ultravox/src-tauri/src/frontmost.rs`
- Modify: `apps/ultravox/src-tauri/Cargo.toml` — add `objc2` deps
...
```bash
cd apps/ultravox/src-tauri && cargo check && cd -
```
...
```bash
git add apps/ultravox/src-tauri
git commit -m "feat(ultravox): get_frontmost_bundle_id command (macOS)"
```

### Task 6.2 — useFrontmostApp hook + display

**Files:**
- Create: `apps/ultravox/src/hooks/useFrontmostApp.ts`
- Modify: `apps/ultravox/src/App.tsx`
...
```bash
git add apps/ultravox/src
git commit -m "feat(ultravox): useFrontmostApp hook with live polling"
```
...
**Goal:** Press `Cmd+Shift+;` from any app → Ultravox registers it as a toggle event. Shows the dev window, starts the recorder via the existing button flow programmatically.

**Outcome:** Hotkey from any app triggers record/stop.

### Task 7.1 — Add global-shortcut plugin

**Files:** `apps/ultravox/src-tauri/Cargo.toml`, `apps/ultravox/src-tauri/src/lib.rs`, `apps/ultravox/src-tauri/capabilities/default.json`
...
cd apps/ultravox/src-tauri && cargo check && cd -
git add -A
git commit -m "feat(ultravox): add global-shortcut plugin"
```

### Task 7.2 — Implement hotkey.rs with toggle event

**Files:** Create `apps/ultravox/src-tauri/src/hotkey.rs`, modify lib.rs
    ...
    app.global_shortcut()
        .on_shortcut(DEFAULT_HOTKEY, move |_app, _shortcut, event| {
            if event.state() == ShortcutState::Pressed {
                let _ = app_for_handler.emit("ultravox://hotkey-toggle", ());
            }
        })
...
`apps/ultravox/src/hooks/useHotkeyEvents.ts`:
...
export function useHotkeyEvents(onToggle: () => void) {
  useEffect(() => {
    const unlistenPromise = listen('ultravox://hotkey-toggle', () => onToggle())
...
Run dev. Click into TextEdit. Press `⌘⇧;`. Expected: Ultravox recorder starts; speak; press again; text appears in TextEdit.

- [ ] **Step 7.2.6: Commit**

```bash
git add apps/ultravox
git commit -m "feat(ultravox): default global hotkey starts/stops recorder"
```
...
**Files:** `apps/ultravox/src-tauri/tauri.conf.json`, `apps/ultravox/index.html` (rename to `settings.html`), `apps/ultravox/pill.html`

- [ ] **Step 8.1.1: Create separate HTML entry points**

Move `apps/ultravox/index.html` → `apps/ultravox/settings.html` (this is the settings shell). Create `apps/ultravox/pill.html`:

```html
<!doctype html>
<html lang="en">
  <head><meta charset="UTF-8" /><title>Ultravox Pill</title></head>
...
`apps/ultravox/src/pill-main.tsx`:
...
Temporarily set `pill` window's `visible: true`, run `pnpm dev:ultravox`, verify a 600x100 transparent rounded window appears. Set back to `false` after verification.

- [ ] **Step 8.1.7: Commit**

```bash
git add -A
git commit -m "feat(ultravox): scaffold pill window + multi-entry vite"
```

### Task 8.2 — window.rs: show/hide pill, position bottom-center

**Files:** Create `apps/ultravox/src-tauri/src/window.rs`, modify lib.rs
...
```bash
git add apps/ultravox/src-tauri
git commit -m "feat(ultravox): show/hide pill at bottom-center"
```
...
export function PillWindow() {
  ...
  useEffect(() => {
    const unlisten = listen('ultravox://hotkey-toggle', () => {
...
```bash
git add apps/ultravox/src
git commit -m "feat(ultravox): pill window state machine + waveform"
```
...
```bash
git add apps/ultravox/src/windows/PillWindow.tsx
git commit -m "feat(ultravox): discard recording with Esc"
```
...
**Files:** `apps/ultravox/src-tauri/Cargo.toml`, `lib.rs`, `capabilities/default.json`, `apps/ultravox/src/lib/store-bridge.ts`
...
```typescript
// in apps/ultravox/package.json deps:
...
```bash
git add -A
git commit -m "feat(ultravox): tauri-plugin-store + store-bridge helpers"
```
...
**Files:** `apps/ultravox/src-tauri/Cargo.toml`, `lib.rs`, `apps/ultravox/src/lib/stronghold-bridge.ts`
...
```rust
.plugin(tauri_plugin_stronghold::Builder::with_argon2(b"ultravox-fixed-salt-v1").build())
```
...
const PASSWORD_KEY = 'ultravox-master-v1'
const VAULT_NAME = 'ultravox-vault'
...
```bash
git add -A
git commit -m "feat(ultravox): stronghold-backed secret storage"
```
...
**Files:** `apps/ultravox/src-tauri/src/hotkey.rs`, `apps/ultravox/src/windows/PillWindow.tsx`
...
.on_shortcut(DEFAULT_HOTKEY, move |_, _, event| {
    if event.state() == ShortcutState::Pressed {
        ...
        let _ = crate::window::show_pill(&app_for_handler);
        let _ = app_for_handler.emit("ultravox://hotkey-toggle", bundle_id);
    }
})
```
...
useEffect(() => {
  const unlisten = listen<string | null>('ultravox://hotkey-toggle', (e) => {
...
```bash
git add apps/ultravox
git commit -m "feat(ultravox): auto-select mode from frontmost app"
```
...
**Files:** `apps/ultravox/src-tauri/Cargo.toml`, `lib.rs`, `apps/ultravox/src-tauri/icons/tray-*.png`
...
```bash
git add -A
git commit -m "feat(ultravox): tray icon + menu with Settings/Quit"
```
...
**Files:** `apps/ultravox/src/windows/SettingsWindow.tsx`, `apps/ultravox/src/main.tsx` (settings entry)
...
For each: `apps/ultravox/src/panels/{Home,Modes,Vocabulary,Configuration,Sound,History}Panel.tsx`:
...
```bash
git add apps/ultravox/src
git commit -m "feat(ultravox): settings shell + 6 panel stubs"
```
...
**Files:** `apps/ultravox/src/panels/VocabularyPanel.tsx`
...
git commit -m "feat(ultravox): VocabularyPanel with persistence"
```

### Task 12.3 — ConfigurationPanel (hotkey + API key + autostart)

**Files:** `apps/ultravox/src/panels/ConfigurationPanel.tsx`
...
git commit -m "feat(ultravox): ConfigurationPanel with hotkey + API key"
```
...
```bash
git add apps/ultravox/src/panels
git commit -m "feat(ultravox): Home/Modes(read-only)/Sound/History panels"
```
...
**Files:** Create `apps/ultravox/src/onboarding/Onboarding.tsx`, modify `SettingsWindow.tsx`
...
rm ~/Library/Application\ Support/com.ultravox.dev/settings.json
pnpm dev:ultravox
```
...
```bash
git add -A
git commit -m "feat(ultravox): first-run 4-step onboarding wizard"
```
...
// in register_default add a second on_shortcut for MODE_HOTKEY emitting "ultravox://mode-overlay"
```
...
useEffect(() => {
  listen('ultravox://mode-overlay', () => setOverlayOpen(true))
}, [])
// inside JSX:
...
```bash
git add -A
git commit -m "feat(ultravox): mid-recording mode switcher overlay"
```
...
**Files:** `apps/ultravox/src-tauri/Cargo.toml`, `apps/ultravox/package.json`, init code

- [ ] **Step 15.1.1: Add Sentry to React side**

```bash
pnpm --filter ultravox add @sentry/react
```
...
```bash
git add -A
git commit -m "feat(ultravox): Sentry on both Rust and React sides"
```
...
```bash
cd apps/ultravox/src-tauri
pnpm tauri signer generate -- -w ~/.tauri/ultravox.key
```
...
"plugins": {
  "updater": {
    "endpoints": ["https://github.com/{owner}/ultravox-releases/releases/latest/download/latest.json"],
...
```bash
git add -A
git commit -m "feat(ultravox): tauri-plugin-updater wired"
```
...
`apps/ultravox/src-tauri/macOS/entitlements.plist`:
...
Expected: produces `apps/ultravox/src-tauri/target/release/bundle/dmg/Ultravox_0.1.0_x64.dmg` (or similar) — signed and notarized.

- [ ] **Step 16.1.5: Verify signature + notarization**

```bash
codesign -dv --verbose=4 apps/ultravox/src-tauri/target/release/bundle/macos/Ultravox.app
spctl -a -vvv apps/ultravox/src-tauri/target/release/bundle/macos/Ultravox.app
```
...
```bash
git add apps/ultravox/src-tauri/macOS apps/ultravox/src-tauri/tauri.conf.json
git commit -m "feat(ultravox): code-sign + notarize macOS bundle"
```
...
**Outcome:** A populated table in `ultravox/known-issues.md` listing every test app and whether paste works, mode auto-switches, and any quirks.

### Task 17.1 — Run the matrix

**Files:** `ultravox/known-issues.md`

- [ ] **Step 17.1.1: Create the file**

```markdown
# Ultravox v1 — Known issues + cross-app paste matrix
...
```bash
git add ultravox/known-issues.md
git commit -m "docs(ultravox): cross-app paste matrix v1"
```
...
- [ ] **Step 18.1.3:** Commit page sources to a separate repo or `apps/ultravox-site/`.
...
- Event names: `ultravox://hotkey-toggle`, `ultravox://mode-overlay` — consistent.
