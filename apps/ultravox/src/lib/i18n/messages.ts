/**
 * The actual catalog content. Adding a new key here causes a TypeScript
 * error in EVERY translation that doesn't have it — exactly the loop-
 * until-green behavior the user asked for. Adding a new language is a
 * one-step change: add an entry under each language object.
 *
 * Translation style notes (German):
 *  - Match the OnboardingWizard.tsx existing `de` voice — informal "du"
 *    address, friendly but not overly casual.
 *  - Keep technical terms English when no clean DE equivalent exists
 *    (Hotkey, Pill, Tray, Cloud) — same as Apple's German UI does.
 *  - Use German quotation marks „…" only in long-form text; UI labels
 *    stay short and unquoted.
 */

import type { Lang, MessageCatalog } from "./catalog";

const en: MessageCatalog = {
  common: {
    back: "Back",
    cancel: "Cancel",
    confirm: "Confirm",
    save: "Save",
    delete: "Delete",
    deleteConfirm: "Click to confirm",
    copy: "Copy",
    copied: "✓ Copied",
    close: "Close",
    open: "Open",
    next: "Next",
    skip: "Skip",
    continueLabel: "Continue",
    refresh: "Refresh",
    settings: "Settings",
    granted: "Granted",
    denied: "Denied",
    notYet: "Not yet",
    on: "On",
    off: "Off",
    loading: "Loading…",
  },
  tray: {
    toggleRecord: "Start / Stop Recording",
    copyLastTranscription: "Copy Last Transcription",
    settingsItem: "Settings…",
    micSettings: "Microphone Settings",
    micOpenSystemSettings: "Open System Settings…",
    micDefault: "Default (system)",
    axSettings: "Accessibility Settings…",
    modeMenu: "Mode",
    quit: "Quit Ultravox",
    versionLabel: (version) => `Version ${version}`,
  },
  appMenu: {
    about: "About Ultravox",
    copyLastTranscription: "Copy Last Transcription",
    hide: "Hide Ultravox",
    hideOthers: "Hide Others",
    showAll: "Show All",
    quit: "Quit Ultravox",
    edit: "Edit",
    undo: "Undo",
    redo: "Redo",
    cut: "Cut",
    copy: "Copy",
    paste: "Paste",
    selectAll: "Select All",
  },
  breadcrumbs: {
    home: "Home",
    modes: "Modes & AI Models",
    vocabulary: "Vocabulary",
    configuration: "Configuration",
    sound: "Sound & Microphone",
    history: "History",
  },
  panels: {
    home: {
      sectionVoice: "Voice",
      navModes: "Modes & AI Models",
      navVocabulary: "Vocabulary",
      navSound: "Sound & Microphone",
      sectionRecording: "Recording",
      hotkeyHelp: "Click a chip to record a new combo. Esc cancels, Backspace clears.",
      recordToggle: "Toggle recording",
      modeSwitcher: "Mode switcher",
      pushToTalk: "Push-to-talk",
      pushToTalkHelp: "Hold the hotkey while speaking instead of toggling. Set a dedicated key combo below.",
      pushToTalkPlaceholder: "v1.5",
      recordingStyleLabel: "Recording style",
      recordingStyleToggle: "Toggle",
      recordingStylePtt: "Push-to-talk",
      pttHotkeyLabel: "Push-to-talk hotkey",
      sectionAppearance: "Appearance",
      themeLabel: "Theme",
      themeLight: "Light",
      themeDark: "Dark",
      themeAuto: "Auto",
      themeOcean: "Ocean",
      themeNight: "Night",
      sectionApp: "App",
      navConfiguration: "Configuration",
      navHistory: "History",
      lastTranscriptionLabel: "Last transcription",
      lastTranscriptionHelp: "Re-copy the most recent transcript into the clipboard if your paste landed in the wrong place. Paste with ⌘V afterward.",
    },
    configuration: {
      sectionAboutYou: "About you",
      sectionAboutYouHelp: "Used by your modes to personalize cleanup (greetings, sign-offs, replacements via {{firstName}} / {{fullName}}). Stored locally only.",
      firstName: "First name",
      lastName: "Last name",
      sectionCleanupBackends: "Cleanup backends",
      sectionCleanupBackendsHelp: "Per-mode cleanup is configured in the Modes panel. This page just shows what backends are available on this Mac.",
      claudeCodeCli: "Claude Code CLI",
      claudeCodeCliInstall: "Install Claude Code CLI to enable on-device cleanup via your local model.",
      claudeCodeCliDetected: (path) => `Detected at ${path}`,
      claudeCodeCliChecking: "Checking…",
      claudeCodeCliAvailable: (version) => `Available · ${version || "v?"}`,
      claudeCodeCliNotInstalled: "Not installed",
      sectionCleanupProviders: "Cleanup providers",
      sectionCleanupProvidersHelp: "Per-mode cleanup runs client-side. Toggle each provider on or off to control whether it appears in the mode editor's provider dropdown and whether the backend may use it.",
      openrouterLabel: "OpenRouter",
      openrouterEnabledHelp: "Bring-your-own OpenRouter key. When on AND a key is saved below, OpenRouter is available as a cleanup provider in the mode editor. When off (or no key), it's hidden from the dropdown and the backend won't use it.",
      openrouterApiKeyLabel: "API key",
      openrouterApiKeyHelp: "Get a key at https://openrouter.ai/keys. Stored in the macOS Keychain — never sent to Ultravox servers, never written to settings.json.",
      openrouterKeySaved: "Saved",
      openrouterKeyNotSet: "Not set",
      openrouterKeyChecking: "Checking…",
      openrouterReplaceKey: "Replace key",
      openrouterSetKey: "Set key",
      openrouterKeyPlaceholder: "sk-or-…",
      openrouterKeySaving: "Saving…",
      openrouterKeyClickAgain: "Click again",
      openrouterKeyRemove: "Remove",
      openrouterKeySavedToast: "Saved to macOS Keychain",
      openrouterKeyRemovedToast: "Removed from Keychain",
      openrouterKeySaveError: (msg) => `Save failed: ${msg}`,
      openrouterKeyRemoveError: (msg) => `Remove failed: ${msg}`,
      claudeCodeEnabledHelp: "Local CLI provides cleanup without API keys. When on AND the CLI is detected on PATH, Claude Code is available as a cleanup provider in the mode editor. When off (or not installed), it's hidden from the dropdown and the backend won't use it.",
      permissionGranted: "✓ Granted",
      permissionWaiting: "Waiting…",
      downloadErrOffline: "You're offline — will resume",
      downloadErrOfflineBanner: "You're offline. Downloads will resume automatically when your connection is back.",
      downloadErrNotFound: "Model file missing",
      downloadErrAuth: "Authentication failed",
      downloadErrDisk: "Out of disk space",
      downloadErrCancelled: "Cancelled",
      sectionOnboarding: "Onboarding wizard",
      sectionOnboardingHelp: "Re-open the setup wizard. Your settings are preserved.",
      launchOnboarding: "Launch",
      diagnosticsFooter: (n) => `${n} entries · newest first`,
      diagnosticsConfirmAgain: "Click to confirm",
      diagnosticsEmpty: "No events yet — try a recording.",
      installedWhisperEmpty: "No Whisper models installed. Pick a variant in a Mode's Transcription Model setting to download one.",
      installedLlmHeader: "Installed LLM models",
      installedLlmEmpty: "No LLM models installed. Pick a variant in a Mode's Processing Model setting to download one.",
      deleteModelTitle: "Delete model file",
      coremlAneBadgeTitle: "CoreML encoder installed — runs on Apple Neural Engine",
      sectionRecordingWindow: "Recording window",
      sectionRecordingWindowHelp: "Choose how the floating pill appears while you're recording.",
      pillStyleLabel: "Style",
      sectionModeSelection: "Mode selection",
      sectionModeSelectionHelp:
        "How Ultravox decides which voice mode to use for each recording.",
      autoModeLabel: "Auto-pick mode by app",
      autoModeHelp:
        "When on, the frontmost app picks the mode (e.g. Mail → Email, VS Code → Code). Falls back to your active mode for unknown apps.",
      sectionTranscription: "Installed transcription",
      sectionPermissions: "Permissions",
      sectionPermissionsHelp: "Required for Ultravox to record audio and paste transcriptions into other apps.",
      micAccess: "Microphone access",
      micGrantedHelp: "Granted — recording works.",
      micDeniedHelp: "Denied — open System Settings → Privacy & Security → Microphone and enable Ultravox.",
      micPromptHelp: "Not yet requested — click Grant Access.",
      micCheckingHelp: "Checking…",
      grantAccess: "Grant Access",
      axAccess: "Accessibility access",
      axGrantedHelp: "Granted — Cmd+V paste works in any app.",
      axIdleHelp: "Required so Ultravox can paste transcribed text into the focused app via Cmd+V.",
      axRequestingHelp: "Waiting for system dialog — click Open System Settings if it didn't appear.",
      sectionRecordings: "Recordings",
      sectionRecordingsHelp: "Optionally save the audio of every recording to your Mac. Files stay local — never uploaded except for the original transcription request. Useful for replay, re-transcribe, or audit.",
      saveAudioLocally: "Save audio recordings locally",
      saveAudioOnHelp: (retentionLabel) =>
        `Auto-deleted after ${retentionLabel}. Folder is configurable below.`,
      saveAudioOffHelp: "Off — recordings are transcribed and discarded, the same as before.",
      folderLabel: "Folder",
      folderDefaultHelp: "Using the default location. Pick a different folder if you'd prefer recordings elsewhere.",
      folderCustomHelp: "Using a custom folder. Reset to put recordings back into Documents.",
      folderChoose: "Choose folder…",
      folderReset: "Reset to default",
      sectionLanguage: "Language",
      sectionLanguageHelp: "Interface language. Spoken language is detected per recording independently.",
      languageLabel: "Display language",
      toggleOffTitle: "Delete existing recordings?",
      toggleOffBody: (count, size) =>
        `You have ${count} saved recording${count === 1 ? "" : "s"} (${size}). Future recordings will no longer be saved — what should we do with the existing files? Recording + transcription continue normally either way; this only affects the audio backup on disk.`,
      toggleOffDelete: "Delete all",
      toggleOffKeep: "Keep on disk",
      sectionLastTranscription: "Last transcription",
      sectionLastTranscriptionHelp: "Where transcribed text goes after each recording — clipboard, history, both, or neither.",
      cacheModeLabel: "Behavior",
      cacheModeHelp: "Controls what happens to each transcript AFTER paste. Independent of audio storage. The transcript is always sent to our server for transcription — this only affects whether the cleaned text is also kept in your local history.",
      cacheModeAutoCopy: "Auto-copy + cache",
      cacheModeCacheOnly: "Cache only (manual copy)",
      cacheModeNoCache: "Don't cache",
      showRecordingsButton: "Show recent recordings",
      autoDeleteAfter: "Delete after",
      retentionNever: "Never",
      retentionDays: (days) => `${days} days`,
      diskUsage: "Disk usage",
      diskUsageEmpty: "0 recordings",
      diskUsageFull: (size) => `${size}`,
      openFolder: "Open folder",
      deleteAll: (count) => `Delete all (${count})`,
      deleteAllConfirm: "Click to delete",
      sectionDiagnostics: "Diagnostics",
      sectionDiagnosticsHelp: "Recent log entries. Useful when reporting an issue.",
      clearLog: "Clear log",
      sectionDangerZone: "Danger Zone",
      resetAll: "Reset all settings",
      resetAllConfirm: "Click again to confirm reset",
      resetAllHelp: "Wipes all preferences, modes, and history. Cannot be undone.",
    },
    modes: {
      sectionRunOnDevice: "Run on-device",
      enableLocalTranscription: "Enable local transcription",
      enableLocalTranscriptionHelp:
        "When on, each mode shows a Transcription Model dropdown to route audio on-device. Off = all modes use cloud.",
      enableLocalCleanup: "Enable local cleanup",
      enableLocalCleanupHelp:
        "When on, modes with provider = Local (on-device LLM) run cleanup on-device. When off, those modes silently fall back to the cloud worker.",
      sectionActiveMode: "Active mode",
      addMode: "Add mode",
      noModes: "No modes configured.",
      unsavedTitle: "Save or discard changes?",
      unsavedBody: "You have unsaved changes in this mode. Save or discard them before leaving.",
      unsavedSave: "Save",
      unsavedDiscard: "Discard",
      downloadModelTitle: "Download model?",
      downloadModelBody: (label, size) => `This mode uses ${label}${size ? ` (${size})` : ""}, which isn't installed yet. Download now?`,
      downloadModelDownload: "Download",
      downloadModelUseCloud: "Use Cloud for now",
      dragHandleTitle: "Drag to reorder",
      duplicateTitle: "Duplicate this mode",
      duplicateAriaLabel: (name) => `Duplicate ${name}`,
      newMode: "New mode",
      newModeCopy: (name) => `New mode — copy of ${name}`,
      configureMode: (name) => `Configure — ${name}`,
      autoModeAppsLabel: "App-Auto mode",
      autoModeAppsHelp: (modeName) =>
        `When you focus one of these apps, recording automatically switches into "${modeName}" mode. Only takes effect when Configuration → Mode Selection → Auto-mode is on.`,
      autoModeAppsEmpty: "No apps yet — add one to auto-switch into this mode.",
      autoModeAppsAdd: "+ Add app…",
      autoModeAppsRemove: (appName) => `Remove ${appName}`,
      autoModeAppsPickerTitle: (modeName) => `Pick an app for ${modeName} mode`,
      editorStyle: "Style",
      editorLanguage: "Language",
      editorAutoCapitalize: "Auto-capitalize",
      editorAutoCapitalizeHelp: "Server-side capitalize after punctuation",
      editorModelsAccordion: "Models",
      editorTranscription: "Transcription",
      editorTranscriptionHelp: "Which Whisper variant runs transcription for this mode. Auto smart-routes based on language.",
      editorProcessingProvider: "Processing Provider",
      editorProcessingProviderHelp: "Cleanup LLM provider",
      editorProcessingAi: "Processing AI",
      editorPromptPlaceholder: "Describe how the AI should clean up your dictated text…",
      editorSlugPlaceholder: "auto",
      editorSlugTitle: "Mode slug — auto-derived from the name. Click here to edit. Locked once saved.",
      editorIdTitle: "Mode ID — frozen on first save, never changes",
      editorIconPickerTitle: "Pick icon",
      editorSave: "Save",
      editorCancel: "Cancel",
      editorDelete: "Delete",
      editorResetToDefault: "Reset to default",
      editorResetTooltip: "You've edited this from the default. Click Reset to restore.",
      editorPromptAccordion: "Prompt",
    },
    sound: {
      roundTripTest: "Round-trip test",
      roundTripTestHelp:
        "Record 2s, send to the worker, show the response. Tests the pipeline in isolation — no hotkey, no paste.",
      testRecordIdle: "Test recording",
      testRecordRecording: "Recording 2s…",
      testRecordTranscribing: "Sending to worker…",
      compareIdle: "Record & compare",
      compareRecording: "Recording 6s…",
      compareRunning: "Comparing…",
      compareRunAgain: "Run again",
      compareWhisperRaw: "Whisper (raw)",
      sectionMicrophone: "Microphone",
      sectionMicrophoneHelp:
        "Ultravox uses your system default microphone. Per-device selection comes in v1.1.",
      sectionCompare: "Compare cleanup quality",
      sectionCompareHelp:
        "Records 6s, then runs the same audio through Whisper raw + four LLM variants in parallel. Use this to pick the right model for your dictation style.",
      sectionInputProcessing: "Input processing",
      autoGain: "Auto-gain",
      autoGainHelp: "Browser auto-adjusts microphone level",
      noiseSuppression: "Noise suppression",
      noiseSuppressionHelp: "Reduce background noise. Mild quality tradeoff.",
      silenceRemoval: "Silence removal",
      silenceRemovalHelp: "Trim silent passages before upload (coming in v1.1).",
      sectionSoundEffects: "Sound effects",
      pauseMusic: "Pause music while recording",
      pauseMusicHelp:
        "Pause Music and Spotify when a recording starts; resume when it stops.",
      duckMusic: "Duck other audio while recording",
      duckMusicHelp:
        "Lower Music/Spotify volume during recording, then restore. Less jarring than pausing if you want to keep listening.",
      duckingDepth: "Ducking depth",
      duckSubtle: "Subtle",
      duckBalanced: "Balanced",
      duckStrong: "Strong",
      duckTooltip: (pct) => `Lower other audio by ${pct}% while recording`,
      chime: "Chime on start/stop",
      chimeHelp: "Brief tone when recording starts and stops",
      chimeVolume: "Chime volume",
      chimeTest: "Test",
      chimeTestStart: "▶ Start",
      chimeTestStop: "▶ Stop",
    },
    vocabulary: {
      title: "Vocabulary",
      help: "Replace words Whisper consistently mishears. Source matches case-insensitively; replacement keeps the case you typed.",
      addEntry: "+ Add entry",
      sourceLabel: "Source",
      replacementLabel: "Replacement",
      empty: "No vocabulary entries yet.",
    },
    history: {
      sectionTitle: (count) => `Recent transcriptions (${count})`,
      clearAll: "Clear all",
      empty: "No transcriptions yet — capture one with the record hotkey.",
      audioBadgeTitle: (size) => `Audio saved (${size})`,
      deleteAudio: "Delete audio",
      footnoteAudio: (count) =>
        `${count} entr${count === 1 ? "y has" : "ies have"} saved audio.`,
      footnoteNoAudio:
        "Audio is only saved when 'Save audio recordings locally' is on in Configuration → Recordings.",
      footnoteCap: "Last 50 transcriptions kept.",
      timeJustNow: "just now",
      timeMinutesAgo: (n) => `${n}m ago`,
      timeHoursAgo: (n) => `${n}h ago`,
      timeDaysAgo: (n) => `${n}d ago`,
      clickToCopy: "Click to copy",
      expand: "Expand",
      collapse: "Collapse",
    },
  },
  pill: {
    discardConfirm: "Discard?",
    discardKeepHint: "Space to keep",
    discardConfirmHint: "Esc to discard",
    transcribing: "Transcribing…",
    silenceClosing: "Nothing to transcribe. Closing…",
    error: "Error",
    dismiss: "Dismiss",
    noSpeech:
      "No speech detected. Move closer to the mic or check your input device.",
    nothingToTranscribe: "Nothing to transcribe.",
    micDenied:
      "Microphone access denied — enable in System Settings → Privacy → Microphone.",
    micNotFound: "No microphone found.",
    couldntStart: "Couldn't start recording.",
    pasteFailed: (msg) =>
      `Paste failed: ${msg} — Accessibility access likely denied.`,
    invalidConstraint:
      "Couldn't configure the mic — restart Ultravox or check System Settings → Sound → Input.",
    silentDescription: "Recording…",
    noAudioCaptured: "No audio captured.",
    audioZeroBytes: "Recording produced 0 bytes — audio format not supported.",
    transcribeFailed: (msg) => `Transcribe failed: ${msg}`,
    openrouterKeyRequired:
      "OpenRouter key required. Settings → Configuration → Cleanup providers, or switch this mode to Claude Code.",
    discardRecording: "Discard recording",
    doubleClickToExpand: "Double-click to expand",
  },
  modeOverlay: {
    title: "Switch mode",
    hint: "↑↓ to choose · Enter to confirm · Esc to cancel",
  },
  confirmDialog: {
    confirm: "Confirm",
    cancel: "Cancel",
  },
  pillStylePicker: {
    classicLabel: "Classic",
    classicDescription: "Full pill with waveform",
    miniLabel: "Mini",
    miniDescription: "Compact dots at top of screen",
  },
  errors: {
    generic: "Something went wrong.",
  },
};

const de: MessageCatalog = {
  common: {
    back: "Zurück",
    cancel: "Abbrechen",
    confirm: "Bestätigen",
    save: "Speichern",
    delete: "Löschen",
    deleteConfirm: "Klicken zum Bestätigen",
    copy: "Kopieren",
    copied: "✓ Kopiert",
    close: "Schließen",
    open: "Öffnen",
    next: "Weiter",
    skip: "Überspringen",
    continueLabel: "Weiter",
    refresh: "Aktualisieren",
    settings: "Einstellungen",
    granted: "Erteilt",
    denied: "Verweigert",
    notYet: "Noch nicht",
    on: "An",
    off: "Aus",
    loading: "Lädt…",
  },
  tray: {
    toggleRecord: "Aufnahme starten/stoppen",
    copyLastTranscription: "Letzte Transkription kopieren",
    settingsItem: "Einstellungen…",
    micSettings: "Mikrofon-Einstellungen",
    micOpenSystemSettings: "Systemeinstellungen öffnen…",
    micDefault: "Standard (System)",
    axSettings: "Bedienungshilfen…",
    modeMenu: "Modus",
    quit: "Ultravox beenden",
    versionLabel: (version) => `Version ${version}`,
  },
  appMenu: {
    about: "Über Ultravox",
    copyLastTranscription: "Letzte Transkription kopieren",
    hide: "Ultravox ausblenden",
    hideOthers: "Andere ausblenden",
    showAll: "Alle einblenden",
    quit: "Ultravox beenden",
    edit: "Bearbeiten",
    undo: "Rückgängig",
    redo: "Wiederherstellen",
    cut: "Ausschneiden",
    copy: "Kopieren",
    paste: "Einsetzen",
    selectAll: "Alles auswählen",
  },
  breadcrumbs: {
    home: "Start",
    modes: "Modi & KI-Modelle",
    vocabulary: "Vokabular",
    configuration: "Konfiguration",
    sound: "Sound & Mikrofon",
    history: "Verlauf",
  },
  panels: {
    home: {
      sectionVoice: "Sprache",
      navModes: "Modi & KI-Modelle",
      navVocabulary: "Vokabular",
      navSound: "Sound & Mikrofon",
      sectionRecording: "Aufnahme",
      hotkeyHelp:
        "Klicke einen Chip, um eine neue Tastenkombination aufzunehmen. Esc bricht ab, Backspace löscht.",
      recordToggle: "Aufnahme-Hotkey",
      modeSwitcher: "Modus-Wechsler",
      pushToTalk: "Push-to-talk",
      pushToTalkHelp:
        "Hotkey gedrückt halten statt zu togglen. Eigene Tastenkombination unten festlegen.",
      pushToTalkPlaceholder: "v1.5",
      recordingStyleLabel: "Aufnahme-Stil",
      recordingStyleToggle: "Toggle",
      recordingStylePtt: "Push-to-talk",
      pttHotkeyLabel: "Push-to-talk Hotkey",
      sectionAppearance: "Erscheinungsbild",
      themeLabel: "Theme",
      themeLight: "Hell",
      themeDark: "Dunkel",
      themeAuto: "Automatisch",
      themeOcean: "Ocean",
      themeNight: "Night",
      sectionApp: "App",
      navConfiguration: "Konfiguration",
      navHistory: "Verlauf",
      lastTranscriptionLabel: "Letzte Transkription",
      lastTranscriptionHelp:
        "Kopiere die letzte Transkription erneut in die Zwischenablage, falls dein Paste irgendwo anders gelandet ist. Danach mit ⌘V einfügen.",
    },
    configuration: {
      sectionAboutYou: "Über dich",
      sectionAboutYouHelp: "Wird von deinen Modi verwendet, um Cleanup zu personalisieren (Begrüßungen, Schlussformeln, Ersetzungen via {{firstName}} / {{fullName}}). Nur lokal gespeichert.",
      firstName: "Vorname",
      lastName: "Nachname",
      sectionCleanupBackends: "Cleanup-Backends",
      sectionCleanupBackendsHelp: "Cleanup pro Modus wird im Modi-Panel konfiguriert. Diese Seite zeigt nur, welche Backends auf diesem Mac verfügbar sind.",
      claudeCodeCli: "Claude Code CLI",
      claudeCodeCliInstall: "Installiere Claude Code CLI, um On-Device-Cleanup mit deinem lokalen Modell zu aktivieren.",
      claudeCodeCliDetected: (path) => `Erkannt unter ${path}`,
      claudeCodeCliChecking: "Prüfe…",
      claudeCodeCliAvailable: (version) => `Verfügbar · ${version || "v?"}`,
      claudeCodeCliNotInstalled: "Nicht installiert",
      sectionCleanupProviders: "Cleanup-Anbieter",
      sectionCleanupProvidersHelp: "Cleanup pro Modus läuft client-seitig. Schalte jeden Anbieter ein oder aus, um zu steuern, ob er im Provider-Dropdown des Modus-Editors erscheint und ob das Backend ihn verwenden darf.",
      openrouterLabel: "OpenRouter",
      openrouterEnabledHelp: "Eigener OpenRouter-Schlüssel. Wenn aktiv UND unten ein Schlüssel gespeichert ist, steht OpenRouter als Cleanup-Anbieter im Modus-Editor zur Verfügung. Bei aus (oder ohne Schlüssel) wird er aus dem Dropdown ausgeblendet und vom Backend nicht verwendet.",
      openrouterApiKeyLabel: "API-Schlüssel",
      openrouterApiKeyHelp: "Schlüssel unter https://openrouter.ai/keys holen. Liegt im macOS-Schlüsselbund — wird nie an Ultravox-Server gesendet und nie in settings.json geschrieben.",
      openrouterKeySaved: "Gespeichert",
      openrouterKeyNotSet: "Nicht gesetzt",
      openrouterKeyChecking: "Prüfe…",
      openrouterReplaceKey: "Schlüssel ersetzen",
      openrouterSetKey: "Schlüssel setzen",
      openrouterKeyPlaceholder: "sk-or-…",
      openrouterKeySaving: "Speichere…",
      openrouterKeyClickAgain: "Erneut klicken",
      openrouterKeyRemove: "Entfernen",
      openrouterKeySavedToast: "Im macOS-Schlüsselbund gespeichert",
      openrouterKeyRemovedToast: "Aus dem Schlüsselbund entfernt",
      openrouterKeySaveError: (msg) => `Speichern fehlgeschlagen: ${msg}`,
      openrouterKeyRemoveError: (msg) => `Entfernen fehlgeschlagen: ${msg}`,
      claudeCodeEnabledHelp: "Lokales CLI für Cleanup ohne API-Schlüssel. Wenn aktiv UND das CLI im PATH erkannt wird, steht Claude Code als Cleanup-Anbieter im Modus-Editor zur Verfügung. Bei aus (oder nicht installiert) wird es aus dem Dropdown ausgeblendet und vom Backend nicht verwendet.",
      permissionGranted: "✓ Erteilt",
      permissionWaiting: "Warte…",
      downloadErrOffline: "Offline — wird fortgesetzt",
      downloadErrOfflineBanner: "Du bist offline. Downloads werden automatisch fortgesetzt, sobald du wieder verbunden bist.",
      downloadErrNotFound: "Modell-Datei nicht gefunden",
      downloadErrAuth: "Authentifizierung fehlgeschlagen",
      downloadErrDisk: "Kein Speicherplatz mehr",
      downloadErrCancelled: "Abgebrochen",
      sectionOnboarding: "Einrichtungs-Assistent",
      sectionOnboardingHelp: "Den Setup-Assistenten erneut öffnen. Deine Einstellungen bleiben erhalten.",
      launchOnboarding: "Starten",
      diagnosticsFooter: (n) => `${n} Einträge · neueste zuerst`,
      diagnosticsConfirmAgain: "Erneut klicken",
      diagnosticsEmpty: "Noch keine Ereignisse — probier eine Aufnahme.",
      installedWhisperEmpty: "Keine Whisper-Modelle installiert. Wähle eine Variante in den Transkriptions-Modell-Einstellungen eines Modus, um sie herunterzuladen.",
      installedLlmHeader: "Installierte LLM-Modelle",
      installedLlmEmpty: "Keine LLM-Modelle installiert. Wähle eine Variante in den Verarbeitungs-Modell-Einstellungen eines Modus, um sie herunterzuladen.",
      deleteModelTitle: "Modelldatei löschen",
      coremlAneBadgeTitle: "CoreML-Encoder installiert — läuft auf Apple Neural Engine",
      sectionRecordingWindow: "Aufnahme-Fenster",
      sectionRecordingWindowHelp:
        "Wähle, wie die schwebende Pille während der Aufnahme aussieht.",
      pillStyleLabel: "Stil",
      sectionModeSelection: "Modusauswahl",
      sectionModeSelectionHelp:
        "Wie Ultravox entscheidet, welcher Voice-Modus für jede Aufnahme verwendet wird.",
      autoModeLabel: "Modus automatisch nach App",
      autoModeHelp:
        "Wenn aktiv, bestimmt die fokussierte App den Modus (z. B. Mail → E-Mail, VS Code → Code). Für unbekannte Apps wird dein aktiver Modus verwendet.",
      sectionTranscription: "Installierte Transkription",
      sectionPermissions: "Berechtigungen",
      sectionPermissionsHelp:
        "Erforderlich, damit Ultravox Audio aufnehmen und Transkriptionen in andere Apps einfügen kann.",
      micAccess: "Mikrofon-Zugriff",
      micGrantedHelp: "Erteilt — Aufnahme funktioniert.",
      micDeniedHelp:
        "Verweigert — öffne Systemeinstellungen → Datenschutz & Sicherheit → Mikrofon und aktiviere Ultravox.",
      micPromptHelp: "Noch nicht angefordert — klicke Zugriff erlauben.",
      micCheckingHelp: "Prüfe…",
      grantAccess: "Zugriff erlauben",
      axAccess: "Bedienungshilfen-Zugriff",
      axGrantedHelp: "Erteilt — Cmd+V Einfügen funktioniert in jeder App.",
      axIdleHelp:
        "Erforderlich, damit Ultravox transkribierten Text per Cmd+V in die fokussierte App einfügen kann.",
      axRequestingHelp:
        "Warte auf Systemdialog — klicke Systemeinstellungen öffnen, falls er nicht erscheint.",
      sectionRecordings: "Aufnahmen",
      sectionRecordingsHelp:
        "Speichere optional das Audio jeder Aufnahme auf deinem Mac. Dateien bleiben lokal — werden nie hochgeladen außer für die ursprüngliche Transkriptions-Anfrage. Nützlich für Wiedergabe, erneute Transkription oder Audit.",
      saveAudioLocally: "Audio-Aufnahmen lokal speichern",
      saveAudioOnHelp: (retentionLabel) =>
        `Automatisch gelöscht nach ${retentionLabel}. Ordner kann unten geändert werden.`,
      saveAudioOffHelp:
        "Aus — Aufnahmen werden transkribiert und wie bisher verworfen.",
      folderLabel: "Ordner",
      folderDefaultHelp:
        "Standard-Speicherort. Wähle einen anderen Ordner, falls du Aufnahmen woanders ablegen möchtest.",
      folderCustomHelp:
        "Eigener Ordner aktiv. Mit Zurücksetzen landen Aufnahmen wieder in Dokumente.",
      folderChoose: "Ordner wählen…",
      folderReset: "Auf Standard zurücksetzen",
      sectionLanguage: "Sprache",
      sectionLanguageHelp:
        "Sprache der Oberfläche. Die gesprochene Sprache wird pro Aufnahme separat erkannt.",
      languageLabel: "Anzeigesprache",
      toggleOffTitle: "Bestehende Aufnahmen löschen?",
      toggleOffBody: (count, size) =>
        `Du hast ${count} gespeicherte Aufnahme${count === 1 ? "" : "n"} (${size}). Neue Aufnahmen werden nicht mehr gespeichert — was soll mit den bestehenden Dateien passieren? Aufnahme und Transkription laufen so oder so weiter; dies betrifft nur die Audio-Sicherung auf der Festplatte.`,
      toggleOffDelete: "Alle löschen + Ordner",
      toggleOffKeep: "Auf Festplatte behalten",
      sectionLastTranscription: "Letzte Transkription",
      sectionLastTranscriptionHelp: "Wohin transkribierter Text nach jeder Aufnahme geht — Zwischenablage, Verlauf, beides oder keines.",
      cacheModeLabel: "Verhalten",
      cacheModeHelp:
        "Was nach dem Einfügen mit jedem Transkript passiert. Unabhängig vom Audio-Speicher. Das Transkript wird zur Transkription so oder so an unseren Server gesendet — diese Einstellung legt nur fest, ob der bereinigte Text auch lokal im Verlauf bleibt.",
      cacheModeAutoCopy: "Auto-Kopie + Cache",
      cacheModeCacheOnly: "Nur Cache (manuell kopieren)",
      cacheModeNoCache: "Nicht zwischenspeichern",
      showRecordingsButton: "Aufnahmen anzeigen",
      autoDeleteAfter: "Löschen nach",
      retentionNever: "Nie",
      retentionDays: (days) => `${days} Tagen`,
      diskUsage: "Speichernutzung",
      diskUsageEmpty: "0 Aufnahmen",
      diskUsageFull: (size) => `${size}`,
      openFolder: "Ordner öffnen",
      deleteAll: (count) => `Alle löschen (${count})`,
      deleteAllConfirm: "Nochmals tippen zum Löschen",
      sectionDiagnostics: "Diagnose",
      sectionDiagnosticsHelp:
        "Letzte Log-Einträge. Hilfreich beim Melden von Problemen.",
      clearLog: "Log leeren",
      sectionDangerZone: "Gefahrenzone",
      resetAll: "Alle Einstellungen zurücksetzen",
      resetAllConfirm: "Erneut klicken zum Bestätigen",
      resetAllHelp:
        "Löscht alle Einstellungen, Modi und den Verlauf. Kann nicht rückgängig gemacht werden.",
    },
    modes: {
      sectionRunOnDevice: "Auf Gerät ausführen",
      enableLocalTranscription: "Lokale Transkription aktivieren",
      enableLocalTranscriptionHelp:
        "Wenn aktiviert, zeigt jeder Modus ein Transkriptions-Modell-Dropdown, um Audio auf deinem Gerät zu verarbeiten. Aus = alle Modi nutzen die Cloud.",
      enableLocalCleanup: "Lokales Cleanup aktivieren",
      enableLocalCleanupHelp:
        "Wenn aktiviert, führen Modi mit Provider = Lokal (On-Device-LLM) das Cleanup auf dem Gerät aus. Bei Aus fallen diese Modi still auf den Cloud-Worker zurück.",
      sectionActiveMode: "Aktiver Modus",
      addMode: "+ Neu",
      noModes: "Keine Modi konfiguriert.",
      unsavedTitle: "Änderungen speichern oder verwerfen?",
      unsavedBody: "Du hast nicht gespeicherte Änderungen in diesem Modus. Speichere sie oder verwirf sie, bevor du wechselst.",
      unsavedSave: "Speichern",
      unsavedDiscard: "Verwerfen",
      downloadModelTitle: "Modell herunterladen?",
      downloadModelBody: (label, size) => `Dieser Modus nutzt ${label}${size ? ` (${size})` : ""}, das noch nicht installiert ist. Jetzt herunterladen?`,
      downloadModelDownload: "Herunterladen",
      downloadModelUseCloud: "Vorerst Cloud nutzen",
      dragHandleTitle: "Zum Neuanordnen ziehen",
      duplicateTitle: "Diesen Modus duplizieren",
      duplicateAriaLabel: (name) => `${name} duplizieren`,
      newMode: "Neuer Modus",
      newModeCopy: (name) => `Neuer Modus — Kopie von ${name}`,
      configureMode: (name) => `Konfigurieren — ${name}`,
      autoModeAppsLabel: "App-Auto-Modus",
      autoModeAppsHelp: (modeName) =>
        `Wenn du eine dieser Apps fokussierst, wechselt die Aufnahme automatisch in den Modus „${modeName}“. Wirkt nur, wenn Konfiguration → Modusauswahl → Auto-Modus aktiv ist.`,
      autoModeAppsEmpty: "Noch keine Apps — füge eine hinzu, um automatisch in diesen Modus zu wechseln.",
      autoModeAppsAdd: "+ App hinzufügen…",
      autoModeAppsRemove: (appName) => `${appName} entfernen`,
      autoModeAppsPickerTitle: (modeName) => `App für Modus „${modeName}“ wählen`,
      editorStyle: "Stil",
      editorLanguage: "Sprache",
      editorAutoCapitalize: "Auto-Großschreibung",
      editorAutoCapitalizeHelp: "Server-seitige Großschreibung nach Satzzeichen",
      editorModelsAccordion: "Modelle",
      editorTranscription: "Transkription",
      editorTranscriptionHelp: "Welche Whisper-Variante für diesen Modus transkribiert. Auto wählt anhand der Sprache.",
      editorProcessingProvider: "Verarbeitungs-Anbieter",
      editorProcessingProviderHelp: "Cleanup-LLM-Anbieter",
      editorProcessingAi: "Verarbeitungs-KI",
      editorPromptPlaceholder: "Beschreibe, wie die KI deinen diktierten Text aufräumen soll…",
      editorSlugPlaceholder: "auto",
      editorSlugTitle: "Modus-Slug — automatisch aus dem Namen abgeleitet. Hier klicken zum Bearbeiten. Gesperrt nach dem Speichern.",
      editorIdTitle: "Modus-ID — beim ersten Speichern fixiert, ändert sich nie",
      editorIconPickerTitle: "Icon wählen",
      editorSave: "Speichern",
      editorCancel: "Abbrechen",
      editorDelete: "Löschen",
      editorResetToDefault: "Auf Standard zurücksetzen",
      editorResetTooltip: "Vom Standard abweichend. Klicke „Zurücksetzen“, um den Standard wiederherzustellen.",
      editorPromptAccordion: "Prompt",
    },
    sound: {
      roundTripTest: "Round-Trip-Test",
      roundTripTestHelp:
        "Nimmt 2s auf, schickt sie an den Worker und zeigt die Antwort. Testet die Pipeline isoliert — ohne Hotkey, ohne Einfügen.",
      testRecordIdle: "Test-Aufnahme",
      testRecordRecording: "Aufnahme 2s…",
      testRecordTranscribing: "Sende an Worker…",
      compareIdle: "Aufnehmen & vergleichen",
      compareRecording: "Aufnahme 6s…",
      compareRunning: "Vergleiche…",
      compareRunAgain: "Erneut ausführen",
      compareWhisperRaw: "Whisper (roh)",
      sectionMicrophone: "Mikrofon",
      sectionMicrophoneHelp:
        "Ultravox nutzt dein Standard-Mikrofon. Geräteauswahl pro Modus kommt in v1.1.",
      sectionCompare: "Cleanup-Qualität vergleichen",
      sectionCompareHelp:
        "Nimmt 6s auf und schickt dasselbe Audio parallel durch Whisper Raw + vier LLM-Varianten. Damit findest du das richtige Modell für deinen Diktier-Stil.",
      sectionInputProcessing: "Eingangs-Verarbeitung",
      autoGain: "Auto-Gain",
      autoGainHelp: "Browser passt Mikrofon-Pegel automatisch an",
      noiseSuppression: "Rauschunterdrückung",
      noiseSuppressionHelp:
        "Reduziert Hintergrundgeräusche. Leichter Qualitätsverlust.",
      silenceRemoval: "Stille entfernen",
      silenceRemovalHelp:
        "Stille Passagen vor dem Upload abschneiden (v1.1)",
      sectionSoundEffects: "Sound-Effekte",
      pauseMusic: "Musik während der Aufnahme pausieren",
      pauseMusicHelp:
        "Pausiert Music und Spotify beim Aufnahmestart; setzt fort beim Stopp.",
      duckMusic: "Anderes Audio während der Aufnahme leiser machen",
      duckMusicHelp:
        "Reduziert Music/Spotify während der Aufnahme und stellt danach wieder her. Weniger störend als komplettes Pausieren.",
      duckingDepth: "Absenk-Tiefe",
      duckSubtle: "Dezent",
      duckBalanced: "Mittel",
      duckStrong: "Stark",
      duckTooltip: (pct) => `Anderes Audio während der Aufnahme um ${pct}% absenken`,
      chime: "Klang bei Start/Stopp",
      chimeHelp: "Kurzer Ton beim Aufnahmestart und -stopp",
      chimeVolume: "Klang-Lautstärke",
      chimeTest: "Test",
      chimeTestStart: "▶ Start",
      chimeTestStop: "▶ Stopp",
    },
    vocabulary: {
      title: "Vokabular",
      help: "Ersetze Wörter, die Whisper konsistent falsch hört. Quelle matcht case-insensitiv; Ersetzung behält die Schreibweise wie eingegeben.",
      addEntry: "+ Eintrag hinzufügen",
      sourceLabel: "Quelle",
      replacementLabel: "Ersetzung",
      empty: "Noch keine Vokabular-Einträge.",
    },
    history: {
      sectionTitle: (count) => `Aktuelle Transkriptionen (${count})`,
      clearAll: "Alle löschen",
      empty: "Noch keine Transkriptionen — nimm eine mit dem Aufnahme-Hotkey auf.",
      audioBadgeTitle: (size) => `Audio gespeichert (${size})`,
      deleteAudio: "Audio löschen",
      footnoteAudio: (count) =>
        `${count} Eintr${count === 1 ? "ag hat" : "äge haben"} gespeichertes Audio.`,
      footnoteNoAudio:
        "Audio wird nur gespeichert, wenn 'Audio-Aufnahmen lokal speichern' in Konfiguration → Aufnahmen aktiv ist.",
      footnoteCap: "Letzte 50 Transkriptionen werden behalten.",
      timeJustNow: "gerade eben",
      timeMinutesAgo: (n) => `vor ${n}m`,
      timeHoursAgo: (n) => `vor ${n}h`,
      timeDaysAgo: (n) => `vor ${n}d`,
      clickToCopy: "Klicken zum Kopieren",
      expand: "Ausklappen",
      collapse: "Einklappen",
    },
  },
  pill: {
    discardConfirm: "Verwerfen?",
    discardKeepHint: "Leertaste zum Behalten",
    discardConfirmHint: "Esc zum Verwerfen",
    transcribing: "Transkribiere…",
    silenceClosing: "Nichts zu transkribieren. Schließe…",
    error: "Fehler",
    dismiss: "Schließen",
    noSpeech:
      "Keine Sprache erkannt. Geh näher ans Mikrofon oder prüfe dein Eingabegerät.",
    nothingToTranscribe: "Nichts zu transkribieren.",
    micDenied:
      "Mikrofon-Zugriff verweigert — aktiviere ihn in Systemeinstellungen → Datenschutz → Mikrofon.",
    micNotFound: "Kein Mikrofon gefunden.",
    couldntStart: "Aufnahme konnte nicht gestartet werden.",
    pasteFailed: (msg) =>
      `Einfügen fehlgeschlagen: ${msg} — Bedienungshilfen-Zugriff vermutlich verweigert.`,
    invalidConstraint:
      "Ungültige Mikrofon-Konfiguration — starte Ultravox neu oder prüfe Systemeinstellungen → Sound → Eingabe.",
    silentDescription: "Aufnahme…",
    noAudioCaptured: "Kein Audio aufgenommen.",
    audioZeroBytes: "Aufnahme ergab 0 Bytes — Audioformat nicht unterstützt.",
    transcribeFailed: (msg) => `Transkription fehlgeschlagen: ${msg}`,
    openrouterKeyRequired:
      "OpenRouter-Key erforderlich. Einstellungen → Konfiguration → Cleanup-Anbieter, oder Modus auf Claude Code wechseln.",
    discardRecording: "Aufnahme verwerfen",
    doubleClickToExpand: "Doppelklick zum Aufklappen",
  },
  modeOverlay: {
    title: "Modus wechseln",
    hint: "↑↓ zum Auswählen · Enter zum Bestätigen · Esc zum Abbrechen",
  },
  confirmDialog: {
    confirm: "Bestätigen",
    cancel: "Abbrechen",
  },
  pillStylePicker: {
    classicLabel: "Klassisch",
    classicDescription: "Volle Pille mit Wellenform",
    miniLabel: "Mini",
    miniDescription: "Kompakte Punkte am oberen Bildschirmrand",
  },
  errors: {
    generic: "Etwas ist schiefgegangen.",
  },
};

const es: MessageCatalog = {
  common: {
    back: "Atrás",
    cancel: "Cancelar",
    confirm: "Confirmar",
    save: "Guardar",
    delete: "Eliminar",
    deleteConfirm: "Haz clic para confirmar",
    copy: "Copiar",
    copied: "✓ Copiado",
    close: "Cerrar",
    open: "Abrir",
    next: "Siguiente",
    skip: "Omitir",
    continueLabel: "Continuar",
    refresh: "Actualizar",
    settings: "Ajustes",
    granted: "Concedido",
    denied: "Denegado",
    notYet: "Aún no",
    on: "Activado",
    off: "Desactivado",
    loading: "Cargando…",
  },
  tray: {
    toggleRecord: "Iniciar/parar grabación",
    copyLastTranscription: "Copiar última transcripción",
    settingsItem: "Ajustes…",
    micSettings: "Ajustes de micrófono",
    micOpenSystemSettings: "Abrir Ajustes del Sistema…",
    micDefault: "Predeterminado (sistema)",
    axSettings: "Ajustes de accesibilidad…",
    modeMenu: "Modo",
    quit: "Salir de Ultravox",
    versionLabel: (version) => `Versión ${version}`,
  },
  appMenu: {
    about: "Acerca de Ultravox",
    copyLastTranscription: "Copiar última transcripción",
    hide: "Ocultar Ultravox",
    hideOthers: "Ocultar otros",
    showAll: "Mostrar todo",
    quit: "Salir de Ultravox",
    edit: "Edición",
    undo: "Deshacer",
    redo: "Rehacer",
    cut: "Cortar",
    copy: "Copiar",
    paste: "Pegar",
    selectAll: "Seleccionar todo",
  },
  breadcrumbs: {
    home: "Inicio",
    modes: "Modos y modelos de IA",
    vocabulary: "Vocabulario",
    configuration: "Configuración",
    sound: "Sonido y micrófono",
    history: "Historial",
  },
  panels: {
    home: {
      sectionVoice: "Voz",
      navModes: "Modos y modelos de IA",
      navVocabulary: "Vocabulario",
      navSound: "Sonido y micrófono",
      sectionRecording: "Grabación",
      hotkeyHelp:
        "Haz clic en un chip para grabar una nueva combinación. Esc cancela, Retroceso borra.",
      recordToggle: "Atajo de grabación",
      modeSwitcher: "Selector de modo",
      pushToTalk: "Mantener para hablar",
      pushToTalkHelp:
        "Mantén pulsado el atajo mientras hablas en lugar de alternar. Define una combinación de teclas dedicada abajo.",
      pushToTalkPlaceholder: "v1.5",
      recordingStyleLabel: "Estilo de grabación",
      recordingStyleToggle: "Alternar",
      recordingStylePtt: "Mantener para hablar",
      pttHotkeyLabel: "Atajo para mantener y hablar",
      sectionAppearance: "Apariencia",
      themeLabel: "Tema",
      themeLight: "Claro",
      themeDark: "Oscuro",
      themeAuto: "Automático",
      themeOcean: "Ocean",
      themeNight: "Night",
      sectionApp: "App",
      navConfiguration: "Configuración",
      navHistory: "Historial",
      lastTranscriptionLabel: "Última transcripción",
      lastTranscriptionHelp:
        "Vuelve a copiar la transcripción más reciente al portapapeles si tu pegado fue al sitio equivocado. Después pega con ⌘V.",
    },
    configuration: {
      sectionAboutYou: "Sobre ti",
      sectionAboutYouHelp:
        "Usado por tus modos para personalizar la limpieza (saludos, despedidas, sustituciones vía {{firstName}} / {{fullName}}). Solo se guarda localmente.",
      firstName: "Nombre",
      lastName: "Apellido",
      sectionCleanupBackends: "Backends de limpieza",
      sectionCleanupBackendsHelp:
        "La limpieza por modo se configura en el panel de Modos. Esta página solo muestra qué backends están disponibles en este Mac.",
      claudeCodeCli: "Claude Code CLI",
      claudeCodeCliInstall:
        "Instala Claude Code CLI para activar la limpieza en el dispositivo con tu modelo local.",
      claudeCodeCliDetected: (path) => `Detectado en ${path}`,
      claudeCodeCliChecking: "Comprobando…",
      claudeCodeCliAvailable: (version) => `Disponible · ${version || "v?"}`,
      claudeCodeCliNotInstalled: "No instalado",
      sectionCleanupProviders: "Proveedores de cleanup",
      sectionCleanupProvidersHelp: "El cleanup por modo se ejecuta en el cliente. Activa o desactiva cada proveedor para controlar si aparece en el menú del editor de modos y si el backend puede usarlo.",
      openrouterLabel: "OpenRouter",
      openrouterEnabledHelp: "Tu propia clave de OpenRouter. Si está activo Y hay una clave guardada abajo, OpenRouter está disponible como proveedor de cleanup en el editor de modos. Si está desactivado (o no hay clave), queda oculto del menú y el backend no lo usa.",
      openrouterApiKeyLabel: "Clave API",
      openrouterApiKeyHelp: "Consigue una clave en https://openrouter.ai/keys. Se guarda en el Llavero de macOS — nunca se envía a los servidores de Ultravox ni se escribe en settings.json.",
      openrouterKeySaved: "Guardada",
      openrouterKeyNotSet: "Sin clave",
      openrouterKeyChecking: "Comprobando…",
      openrouterReplaceKey: "Reemplazar clave",
      openrouterSetKey: "Añadir clave",
      openrouterKeyPlaceholder: "sk-or-…",
      openrouterKeySaving: "Guardando…",
      openrouterKeyClickAgain: "Haz clic otra vez",
      openrouterKeyRemove: "Quitar",
      openrouterKeySavedToast: "Guardada en el Llavero de macOS",
      openrouterKeyRemovedToast: "Eliminada del Llavero",
      openrouterKeySaveError: (msg) => `No se pudo guardar: ${msg}`,
      openrouterKeyRemoveError: (msg) => `No se pudo eliminar: ${msg}`,
      claudeCodeEnabledHelp: "CLI local que hace cleanup sin claves API. Si está activo Y se detecta el CLI en el PATH, Claude Code está disponible como proveedor de cleanup en el editor de modos. Si está desactivado (o no instalado), queda oculto del menú y el backend no lo usa.",
      permissionGranted: "✓ Concedido",
      permissionWaiting: "Esperando…",
      downloadErrOffline: "Sin conexión — se reanudará",
      downloadErrOfflineBanner: "Estás sin conexión. Las descargas se reanudarán automáticamente cuando recuperes la conexión.",
      downloadErrNotFound: "Archivo de modelo no encontrado",
      downloadErrAuth: "Autenticación fallida",
      downloadErrDisk: "Sin espacio en disco",
      downloadErrCancelled: "Cancelada",
      sectionOnboarding: "Asistente de inicio",
      sectionOnboardingHelp:
        "Vuelve a abrir el asistente de configuración. Tus ajustes se conservan.",
      launchOnboarding: "Iniciar",
      diagnosticsFooter: (n) => `${n} entradas · más recientes arriba`,
      diagnosticsConfirmAgain: "Haz clic otra vez",
      diagnosticsEmpty: "Aún no hay eventos — prueba con una grabación.",
      installedWhisperEmpty:
        "No hay modelos de Whisper instalados. Elige una variante en el ajuste de Modelo de Transcripción de un modo para descargar uno.",
      installedLlmHeader: "Modelos LLM instalados",
      installedLlmEmpty:
        "No hay modelos LLM instalados. Elige una variante en el ajuste de Modelo de Procesamiento de un modo para descargar uno.",
      deleteModelTitle: "Eliminar archivo de modelo",
      coremlAneBadgeTitle:
        "Encoder CoreML instalado — se ejecuta en Apple Neural Engine",
      sectionRecordingWindow: "Ventana de grabación",
      sectionRecordingWindowHelp:
        "Elige cómo aparece la píldora flotante mientras grabas.",
      pillStyleLabel: "Estilo",
      sectionModeSelection: "Selección de modo",
      sectionModeSelectionHelp:
        "Cómo decide Ultravox qué modo de voz usar en cada grabación.",
      autoModeLabel: "Modo automático por app",
      autoModeHelp:
        "Cuando está activo, la app en primer plano elige el modo (p. ej. Mail → Email, VS Code → Code). Para apps desconocidas se usa tu modo activo.",
      sectionTranscription: "Transcripción instalada",
      sectionPermissions: "Permisos",
      sectionPermissionsHelp:
        "Necesarios para que Ultravox pueda grabar audio y pegar transcripciones en otras apps.",
      micAccess: "Acceso al micrófono",
      micGrantedHelp: "Concedido — la grabación funciona.",
      micDeniedHelp:
        "Denegado — abre Ajustes del Sistema → Privacidad y seguridad → Micrófono y activa Ultravox.",
      micPromptHelp: "Aún no solicitado — haz clic en Conceder acceso.",
      micCheckingHelp: "Comprobando…",
      grantAccess: "Conceder acceso",
      axAccess: "Acceso de accesibilidad",
      axGrantedHelp: "Concedido — el pegado con Cmd+V funciona en cualquier app.",
      axIdleHelp:
        "Necesario para que Ultravox pueda pegar el texto transcrito en la app con foco mediante Cmd+V.",
      axRequestingHelp:
        "Esperando al diálogo del sistema — haz clic en Abrir Ajustes del Sistema si no aparece.",
      sectionRecordings: "Grabaciones",
      sectionRecordingsHelp:
        "Guarda opcionalmente el audio de cada grabación en tu Mac. Los archivos permanecen locales — nunca se suben excepto para la petición original de transcripción. Útil para reproducir, re-transcribir o auditar.",
      saveAudioLocally: "Guardar grabaciones de audio localmente",
      saveAudioOnHelp: (retentionLabel) =>
        `Eliminado automáticamente tras ${retentionLabel}. La carpeta se puede cambiar abajo.`,
      saveAudioOffHelp:
        "Desactivado — las grabaciones se transcriben y se descartan como hasta ahora.",
      folderLabel: "Carpeta",
      folderDefaultHelp:
        "Usando la ubicación predeterminada. Elige otra carpeta si prefieres guardar las grabaciones en otro sitio.",
      folderCustomHelp:
        "Usando una carpeta personalizada. Restablece para volver a guardar las grabaciones en Documentos.",
      folderChoose: "Elegir carpeta…",
      folderReset: "Restablecer al valor predeterminado",
      sectionLanguage: "Idioma",
      sectionLanguageHelp:
        "Idioma de la interfaz. El idioma hablado se detecta de forma independiente en cada grabación.",
      languageLabel: "Idioma de la interfaz",
      toggleOffTitle: "¿Eliminar las grabaciones existentes?",
      toggleOffBody: (count, size) =>
        `Tienes ${count} grabación${count === 1 ? "" : "es"} guardada${count === 1 ? "" : "s"} (${size}). Las futuras grabaciones ya no se guardarán — ¿qué hacemos con los archivos existentes? La grabación y la transcripción siguen funcionando igual; esto solo afecta a la copia de seguridad de audio en el disco.`,
      toggleOffDelete: "Eliminar todo + carpeta",
      toggleOffKeep: "Conservar en el disco",
      sectionLastTranscription: "Última transcripción",
      sectionLastTranscriptionHelp:
        "Adónde va el texto transcrito tras cada grabación — al portapapeles, al historial, a ambos o a ninguno.",
      cacheModeLabel: "Comportamiento",
      cacheModeHelp:
        "Controla qué pasa con cada transcripción DESPUÉS de pegarla. Independiente del almacenamiento de audio. La transcripción siempre se envía a nuestro servidor para procesarla — esto solo afecta a si el texto limpio se conserva también en tu historial local.",
      cacheModeAutoCopy: "Copiar y guardar en caché",
      cacheModeCacheOnly: "Solo caché (copiar a mano)",
      cacheModeNoCache: "No guardar nada",
      showRecordingsButton: "Mostrar grabaciones recientes",
      autoDeleteAfter: "Eliminar tras",
      retentionNever: "Nunca",
      retentionDays: (days) => `${days} días`,
      diskUsage: "Uso de disco",
      diskUsageEmpty: "0 grabaciones",
      diskUsageFull: (size) => `${size}`,
      openFolder: "Abrir carpeta",
      deleteAll: (count) => `Eliminar todo (${count})`,
      deleteAllConfirm: "Toca otra vez para eliminar",
      sectionDiagnostics: "Diagnóstico",
      sectionDiagnosticsHelp:
        "Últimas entradas del registro. Útil al reportar un problema.",
      clearLog: "Vaciar registro",
      sectionDangerZone: "Zona de peligro",
      resetAll: "Restablecer todos los ajustes",
      resetAllConfirm: "Haz clic otra vez para confirmar",
      resetAllHelp:
        "Borra todas las preferencias, modos e historial. No se puede deshacer.",
    },
    modes: {
      sectionRunOnDevice: "Ejecutar en el dispositivo",
      enableLocalTranscription: "Activar transcripción local",
      enableLocalTranscriptionHelp:
        "Cuando está activado, cada modo muestra un desplegable de Modelo de Transcripción para procesar el audio en el dispositivo. Desactivado = todos los modos usan la nube.",
      enableLocalCleanup: "Activar limpieza local",
      enableLocalCleanupHelp:
        "Cuando está activado, los modos con proveedor = Local (LLM en el dispositivo) ejecutan la limpieza localmente. Cuando está desactivado, esos modos recurren silenciosamente al worker en la nube.",
      sectionActiveMode: "Modo activo",
      addMode: "+ Nuevo",
      noModes: "Ningún modo configurado.",
      unsavedTitle: "¿Guardar o descartar cambios?",
      unsavedBody:
        "Tienes cambios sin guardar en este modo. Guárdalos o descártalos antes de salir.",
      unsavedSave: "Guardar",
      unsavedDiscard: "Descartar",
      downloadModelTitle: "¿Descargar modelo?",
      downloadModelBody: (label, size) =>
        `Este modo usa ${label}${size ? ` (${size})` : ""}, que aún no está instalado. ¿Descargar ahora?`,
      downloadModelDownload: "Descargar",
      downloadModelUseCloud: "Usar la nube por ahora",
      dragHandleTitle: "Arrastra para reordenar",
      duplicateTitle: "Duplicar este modo",
      duplicateAriaLabel: (name) => `Duplicar ${name}`,
      newMode: "Nuevo modo",
      newModeCopy: (name) => `Nuevo modo — copia de ${name}`,
      configureMode: (name) => `Configurar — ${name}`,
      autoModeAppsLabel: "Modo App-Auto",
      autoModeAppsHelp: (modeName) =>
        `Cuando enfoques una de estas apps, la grabación cambiará automáticamente al modo «${modeName}». Solo se aplica si Configuración → Selección de modo → Modo automático está activo.`,
      autoModeAppsEmpty: "Aún no hay apps — añade una para activar automáticamente este modo.",
      autoModeAppsAdd: "+ Añadir app…",
      autoModeAppsRemove: (appName) => `Quitar ${appName}`,
      autoModeAppsPickerTitle: (modeName) => `Elige una app para el modo ${modeName}`,
      editorStyle: "Estilo",
      editorLanguage: "Idioma",
      editorAutoCapitalize: "Mayúscula automática",
      editorAutoCapitalizeHelp: "Mayúscula automática después de cada signo de puntuación",
      editorModelsAccordion: "Modelos",
      editorTranscription: "Transcripción",
      editorTranscriptionHelp: "Qué variante de Whisper transcribe en este modo. Auto enruta según el idioma.",
      editorProcessingProvider: "Proveedor de procesamiento",
      editorProcessingProviderHelp: "Proveedor del LLM de cleanup",
      editorProcessingAi: "IA de procesamiento",
      editorPromptPlaceholder: "Describe cómo debe la IA limpiar tu texto dictado…",
      editorSlugPlaceholder: "auto",
      editorSlugTitle: "Slug del modo — derivado automáticamente del nombre. Haz clic aquí para editar. Bloqueado tras guardar.",
      editorIdTitle: "ID del modo — fijado al guardar por primera vez, nunca cambia",
      editorIconPickerTitle: "Elegir icono",
      editorSave: "Guardar",
      editorCancel: "Cancelar",
      editorDelete: "Eliminar",
      editorResetToDefault: "Restablecer al predeterminado",
      editorResetTooltip: "Has modificado esto respecto al predeterminado. Haz clic en Restablecer para restaurar.",
      editorPromptAccordion: "Prompt",
    },
    sound: {
      roundTripTest: "Prueba de ida y vuelta",
      roundTripTestHelp:
        "Graba 2s, envía al worker y muestra la respuesta. Prueba la pipeline aislada — sin atajo, sin pegado.",
      testRecordIdle: "Grabación de prueba",
      testRecordRecording: "Grabando 2s…",
      testRecordTranscribing: "Enviando al worker…",
      compareIdle: "Grabar y comparar",
      compareRecording: "Grabando 6s…",
      compareRunning: "Comparando…",
      compareRunAgain: "Ejecutar otra vez",
      compareWhisperRaw: "Whisper (en bruto)",
      sectionMicrophone: "Micrófono",
      sectionMicrophoneHelp:
        "Ultravox usa el micrófono predeterminado del sistema. La selección por dispositivo llega en v1.1.",
      sectionCompare: "Comparar calidad de limpieza",
      sectionCompareHelp:
        "Graba 6s y luego envía el mismo audio en paralelo por Whisper en bruto + cuatro variantes de LLM. Usa esto para elegir el modelo adecuado para tu estilo de dictado.",
      sectionInputProcessing: "Procesamiento de entrada",
      autoGain: "Ganancia automática",
      autoGainHelp: "El navegador ajusta el nivel del micrófono automáticamente",
      noiseSuppression: "Supresión de ruido",
      noiseSuppressionHelp:
        "Reduce el ruido de fondo. Ligera pérdida de calidad.",
      silenceRemoval: "Eliminación de silencios",
      silenceRemovalHelp:
        "Recortar pasajes silenciosos antes de la subida (v1.1)",
      sectionSoundEffects: "Efectos de sonido",
      pauseMusic: "Pausar música mientras grabas",
      pauseMusicHelp:
        "Pausa Music y Spotify al iniciar una grabación; reanuda al pararla.",
      duckMusic: "Bajar el otro audio mientras grabas",
      duckMusicHelp:
        "Baja el volumen de Music/Spotify durante la grabación y lo restaura después. Menos brusco que pausar si quieres seguir escuchando.",
      duckingDepth: "Profundidad de atenuación",
      duckSubtle: "Sutil",
      duckBalanced: "Equilibrado",
      duckStrong: "Fuerte",
      duckTooltip: (pct) => `Bajar el otro audio un ${pct}% durante la grabación`,
      chime: "Tono al iniciar/parar",
      chimeHelp: "Un tono breve al empezar y al parar la grabación",
      chimeVolume: "Volumen del tono",
      chimeTest: "Probar",
      chimeTestStart: "▶ Iniciar",
      chimeTestStop: "▶ Parar",
    },
    vocabulary: {
      title: "Vocabulario",
      help: "Reemplaza palabras que Whisper malinterpreta siempre. La fuente coincide sin distinguir mayúsculas; la sustitución conserva la grafía que escribas.",
      addEntry: "+ Añadir entrada",
      sourceLabel: "Fuente",
      replacementLabel: "Sustitución",
      empty: "Aún no hay entradas de vocabulario.",
    },
    history: {
      sectionTitle: (count) => `Transcripciones recientes (${count})`,
      clearAll: "Borrar todo",
      empty:
        "Aún no hay transcripciones — captura una con el atajo de grabación.",
      audioBadgeTitle: (size) => `Audio guardado (${size})`,
      deleteAudio: "Eliminar audio",
      footnoteAudio: (count) =>
        `${count} entrada${count === 1 ? "" : "s"} tiene${count === 1 ? "" : "n"} audio guardado.`,
      footnoteNoAudio:
        "El audio solo se guarda cuando 'Guardar grabaciones de audio localmente' está activado en Configuración → Grabaciones.",
      footnoteCap: "Se conservan las últimas 50 transcripciones.",
      timeJustNow: "ahora mismo",
      timeMinutesAgo: (n) => `hace ${n} min`,
      timeHoursAgo: (n) => `hace ${n} h`,
      timeDaysAgo: (n) => `hace ${n} d`,
      clickToCopy: "Haz clic para copiar",
      expand: "Expandir",
      collapse: "Contraer",
    },
  },
  pill: {
    discardConfirm: "¿Descartar?",
    discardKeepHint: "Espacio para conservar",
    discardConfirmHint: "Esc para descartar",
    transcribing: "Transcribiendo…",
    silenceClosing: "Nada que transcribir. Cerrando…",
    error: "Error",
    dismiss: "Cerrar",
    noSpeech:
      "No se ha detectado voz. Acércate al micrófono o comprueba tu dispositivo de entrada.",
    nothingToTranscribe: "Nada que transcribir.",
    micDenied:
      "Acceso al micrófono denegado — actívalo en Ajustes del Sistema → Privacidad → Micrófono.",
    micNotFound: "No se ha encontrado ningún micrófono.",
    couldntStart: "No se ha podido iniciar la grabación.",
    pasteFailed: (msg) =>
      `Pegado fallido: ${msg} — probablemente falta acceso de accesibilidad.`,
    invalidConstraint:
      "Configuración de micrófono inválida — reinicia Ultravox o comprueba Ajustes del Sistema → Sonido → Entrada.",
    silentDescription: "Grabando…",
    noAudioCaptured: "No se capturó audio.",
    audioZeroBytes: "La grabación produjo 0 bytes — formato de audio no compatible.",
    transcribeFailed: (msg) => `Error al transcribir: ${msg}`,
    openrouterKeyRequired:
      "Se necesita clave de OpenRouter. Ajustes → Configuración → Proveedores de limpieza, o cambia este modo a Claude Code.",
    discardRecording: "Descartar grabación",
    doubleClickToExpand: "Doble clic para ampliar",
  },
  modeOverlay: {
    title: "Cambiar de modo",
    hint: "↑↓ para elegir · Intro para confirmar · Esc para cancelar",
  },
  confirmDialog: {
    confirm: "Confirmar",
    cancel: "Cancelar",
  },
  pillStylePicker: {
    classicLabel: "Clásica",
    classicDescription: "Píldora completa con forma de onda",
    miniLabel: "Mini",
    miniDescription: "Puntos compactos en la parte superior de la pantalla",
  },
  errors: {
    generic: "Algo ha salido mal.",
  },
};

const sv: MessageCatalog = {
  common: {
    back: "Tillbaka",
    cancel: "Avbryt",
    confirm: "Bekräfta",
    save: "Spara",
    delete: "Radera",
    deleteConfirm: "Klicka för att bekräfta",
    copy: "Kopiera",
    copied: "✓ Kopierat",
    close: "Stäng",
    open: "Öppna",
    next: "Nästa",
    skip: "Hoppa över",
    continueLabel: "Fortsätt",
    refresh: "Uppdatera",
    settings: "Inställningar",
    granted: "Beviljad",
    denied: "Nekad",
    notYet: "Inte än",
    on: "På",
    off: "Av",
    loading: "Laddar…",
  },
  tray: {
    toggleRecord: "Starta/stoppa inspelning",
    copyLastTranscription: "Kopiera senaste transkription",
    settingsItem: "Inställningar…",
    micSettings: "Mikrofoninställningar",
    micOpenSystemSettings: "Öppna Systeminställningar…",
    micDefault: "Standard (system)",
    axSettings: "Hjälpmedelsinställningar…",
    modeMenu: "Läge",
    quit: "Avsluta Ultravox",
    versionLabel: (version) => `Version ${version}`,
  },
  appMenu: {
    about: "Om Ultravox",
    copyLastTranscription: "Kopiera senaste transkription",
    hide: "Göm Ultravox",
    hideOthers: "Göm andra",
    showAll: "Visa alla",
    quit: "Avsluta Ultravox",
    edit: "Redigera",
    undo: "Ångra",
    redo: "Gör om",
    cut: "Klipp ut",
    copy: "Kopiera",
    paste: "Klistra in",
    selectAll: "Markera allt",
  },
  breadcrumbs: {
    home: "Hem",
    modes: "Lägen & AI-modeller",
    vocabulary: "Ordlista",
    configuration: "Konfiguration",
    sound: "Ljud & mikrofon",
    history: "Historik",
  },
  panels: {
    home: {
      sectionVoice: "Röst",
      navModes: "Lägen & AI-modeller",
      navVocabulary: "Ordlista",
      navSound: "Ljud & mikrofon",
      sectionRecording: "Inspelning",
      hotkeyHelp:
        "Klicka på en bricka för att spela in en ny kombination. Esc avbryter, Backsteg rensar.",
      recordToggle: "Inspelnings-snabbtangent",
      modeSwitcher: "Lägesväxlare",
      pushToTalk: "Tryck och tala",
      pushToTalkHelp:
        "Håll ned snabbtangenten medan du pratar istället för att växla. Ange en egen tangentkombination nedan.",
      pushToTalkPlaceholder: "v1.5",
      recordingStyleLabel: "Inspelningsstil",
      recordingStyleToggle: "Växla",
      recordingStylePtt: "Tryck och tala",
      pttHotkeyLabel: "Tangent för tryck och tala",
      sectionAppearance: "Utseende",
      themeLabel: "Tema",
      themeLight: "Ljust",
      themeDark: "Mörkt",
      themeAuto: "Automatiskt",
      themeOcean: "Ocean",
      themeNight: "Night",
      sectionApp: "App",
      navConfiguration: "Konfiguration",
      navHistory: "Historik",
      lastTranscriptionLabel: "Senaste transkription",
      lastTranscriptionHelp:
        "Kopiera senaste transkriptionen till urklipp igen om din inklistring hamnade på fel ställe. Klistra sedan in med ⌘V.",
    },
    configuration: {
      sectionAboutYou: "Om dig",
      sectionAboutYouHelp:
        "Används av dina lägen för att personifiera städningen (hälsningsfraser, avslutningar, ersättningar via {{firstName}} / {{fullName}}). Sparas bara lokalt.",
      firstName: "Förnamn",
      lastName: "Efternamn",
      sectionCleanupBackends: "Städningsmotorer",
      sectionCleanupBackendsHelp:
        "Städning per läge konfigureras i Lägen-panelen. Den här sidan visar bara vilka backends som finns på den här Macen.",
      claudeCodeCli: "Claude Code CLI",
      claudeCodeCliInstall:
        "Installera Claude Code CLI för att aktivera städning på enheten via din lokala modell.",
      claudeCodeCliDetected: (path) => `Hittad vid ${path}`,
      claudeCodeCliChecking: "Kontrollerar…",
      claudeCodeCliAvailable: (version) => `Tillgänglig · ${version || "v?"}`,
      claudeCodeCliNotInstalled: "Inte installerad",
      sectionCleanupProviders: "Cleanup-leverantörer",
      sectionCleanupProvidersHelp: "Cleanup per läge körs på klienten. Slå på eller av varje leverantör för att styra om den visas i lägesredigerarens leverantörsmeny och om backend får använda den.",
      openrouterLabel: "OpenRouter",
      openrouterEnabledHelp: "Din egen OpenRouter-nyckel. När påslagen OCH en nyckel finns sparad nedan är OpenRouter tillgänglig som cleanup-leverantör i lägesredigeraren. När avstängd (eller utan nyckel) döljs den från menyn och backend använder den inte.",
      openrouterApiKeyLabel: "API-nyckel",
      openrouterApiKeyHelp: "Hämta en nyckel på https://openrouter.ai/keys. Lagras i macOS-nyckelringen — skickas aldrig till Ultravox-servrar och skrivs aldrig till settings.json.",
      openrouterKeySaved: "Sparad",
      openrouterKeyNotSet: "Inte satt",
      openrouterKeyChecking: "Kontrollerar…",
      openrouterReplaceKey: "Ersätt nyckel",
      openrouterSetKey: "Lägg till nyckel",
      openrouterKeyPlaceholder: "sk-or-…",
      openrouterKeySaving: "Sparar…",
      openrouterKeyClickAgain: "Klicka igen",
      openrouterKeyRemove: "Ta bort",
      openrouterKeySavedToast: "Sparad i macOS-nyckelringen",
      openrouterKeyRemovedToast: "Borttagen från nyckelringen",
      openrouterKeySaveError: (msg) => `Kunde inte spara: ${msg}`,
      openrouterKeyRemoveError: (msg) => `Kunde inte ta bort: ${msg}`,
      claudeCodeEnabledHelp: "Lokalt CLI som gör cleanup utan API-nycklar. När påslagen OCH CLI:t hittas i PATH är Claude Code tillgänglig som cleanup-leverantör i lägesredigeraren. När avstängd (eller inte installerad) döljs den från menyn och backend använder den inte.",
      permissionGranted: "✓ Beviljat",
      permissionWaiting: "Väntar…",
      downloadErrOffline: "Offline — återupptas",
      downloadErrOfflineBanner: "Du är offline. Nedladdningar återupptas automatiskt när anslutningen är tillbaka.",
      downloadErrNotFound: "Modellfilen saknas",
      downloadErrAuth: "Autentisering misslyckades",
      downloadErrDisk: "Slut på diskutrymme",
      downloadErrCancelled: "Avbruten",
      sectionOnboarding: "Introduktionsguide",
      sectionOnboardingHelp:
        "Öppna konfigurationsguiden igen. Dina inställningar bevaras.",
      launchOnboarding: "Starta",
      diagnosticsFooter: (n) => `${n} poster · senaste först`,
      diagnosticsConfirmAgain: "Klicka igen",
      diagnosticsEmpty: "Inga händelser än — testa en inspelning.",
      installedWhisperEmpty:
        "Inga Whisper-modeller installerade. Välj en variant i ett läges Transkriptionsmodell-inställning för att ladda ner en.",
      installedLlmHeader: "Installerade LLM-modeller",
      installedLlmEmpty:
        "Inga LLM-modeller installerade. Välj en variant i ett läges Bearbetningsmodell-inställning för att ladda ner en.",
      deleteModelTitle: "Radera modellfil",
      coremlAneBadgeTitle:
        "CoreML-encoder installerad — körs på Apple Neural Engine",
      sectionRecordingWindow: "Inspelningsfönster",
      sectionRecordingWindowHelp:
        "Välj hur den flytande pillerformen ser ut medan du spelar in.",
      pillStyleLabel: "Stil",
      sectionModeSelection: "Lägesval",
      sectionModeSelectionHelp:
        "Hur Ultravox väljer röstläge för varje inspelning.",
      autoModeLabel: "Välj läge per app",
      autoModeHelp:
        "När påslagen väljer den aktiva appen läget (t.ex. Mail → Email, VS Code → Code). För okända appar används ditt aktiva läge.",
      sectionTranscription: "Installerad transkription",
      sectionPermissions: "Behörigheter",
      sectionPermissionsHelp:
        "Krävs för att Ultravox ska kunna spela in ljud och klistra in transkriptioner i andra appar.",
      micAccess: "Mikrofonåtkomst",
      micGrantedHelp: "Beviljad — inspelning fungerar.",
      micDeniedHelp:
        "Nekad — öppna Systeminställningar → Integritet och säkerhet → Mikrofon och aktivera Ultravox.",
      micPromptHelp: "Inte begärd än — klicka Bevilja åtkomst.",
      micCheckingHelp: "Kontrollerar…",
      grantAccess: "Bevilja åtkomst",
      axAccess: "Hjälpmedelsåtkomst",
      axGrantedHelp: "Beviljad — Cmd+V inklistring fungerar i alla appar.",
      axIdleHelp:
        "Krävs för att Ultravox ska kunna klistra in transkriberad text i den fokuserade appen via Cmd+V.",
      axRequestingHelp:
        "Väntar på systemdialog — klicka Öppna Systeminställningar om den inte dyker upp.",
      sectionRecordings: "Inspelningar",
      sectionRecordingsHelp:
        "Spara valfritt ljudet från varje inspelning på din Mac. Filer förblir lokala — laddas aldrig upp förutom för själva transkriptionsförfrågan. Praktiskt för uppspelning, ny transkription eller granskning.",
      saveAudioLocally: "Spara ljudinspelningar lokalt",
      saveAudioOnHelp: (retentionLabel) =>
        `Tas automatiskt bort efter ${retentionLabel}. Mappen kan ändras nedan.`,
      saveAudioOffHelp:
        "Av — inspelningar transkriberas och kastas som idag.",
      folderLabel: "Mapp",
      folderDefaultHelp:
        "Använder standardplatsen. Välj en annan mapp om du vill spara inspelningar någon annanstans.",
      folderCustomHelp:
        "Använder en egen mapp. Återställ för att flytta tillbaka inspelningarna till Dokument.",
      folderChoose: "Välj mapp…",
      folderReset: "Återställ till standard",
      sectionLanguage: "Språk",
      sectionLanguageHelp:
        "Gränssnittets språk. Det talade språket identifieras per inspelning separat.",
      languageLabel: "Visningsspråk",
      toggleOffTitle: "Radera befintliga inspelningar?",
      toggleOffBody: (count, size) =>
        `Du har ${count} sparad inspelning${count === 1 ? "" : "ar"} (${size}). Framtida inspelningar sparas inte längre — vad ska vi göra med de befintliga filerna? Inspelning och transkription fungerar som vanligt oavsett; det här påverkar bara ljudsäkerhetskopian på disken.`,
      toggleOffDelete: "Radera allt + mapp",
      toggleOffKeep: "Behåll på disken",
      sectionLastTranscription: "Senaste transkription",
      sectionLastTranscriptionHelp:
        "Var den transkriberade texten hamnar efter varje inspelning — urklipp, historik, båda eller ingen.",
      cacheModeLabel: "Beteende",
      cacheModeHelp:
        "Styr vad som händer med varje transkription EFTER inklistring. Oberoende av ljudlagring. Transkriptionen skickas alltid till vår server för bearbetning — det här påverkar bara om den städade texten också sparas i din lokala historik.",
      cacheModeAutoCopy: "Kopiera och spara i cache",
      cacheModeCacheOnly: "Endast cache (kopiera manuellt)",
      cacheModeNoCache: "Spara inget",
      showRecordingsButton: "Visa senaste inspelningar",
      autoDeleteAfter: "Radera efter",
      retentionNever: "Aldrig",
      retentionDays: (days) => `${days} dagar`,
      diskUsage: "Diskanvändning",
      diskUsageEmpty: "0 inspelningar",
      diskUsageFull: (size) => `${size}`,
      openFolder: "Öppna mapp",
      deleteAll: (count) => `Radera allt (${count})`,
      deleteAllConfirm: "Tryck igen för att radera",
      sectionDiagnostics: "Diagnostik",
      sectionDiagnosticsHelp:
        "Senaste loggposter. Bra när du rapporterar ett problem.",
      clearLog: "Töm logg",
      sectionDangerZone: "Riskzon",
      resetAll: "Återställ alla inställningar",
      resetAllConfirm: "Klicka igen för att bekräfta",
      resetAllHelp:
        "Raderar alla inställningar, lägen och historik. Kan inte ångras.",
    },
    modes: {
      sectionRunOnDevice: "Kör på enheten",
      enableLocalTranscription: "Aktivera lokal transkription",
      enableLocalTranscriptionHelp:
        "När påslaget visar varje läge en Transkriptionsmodell-meny för att köra ljudet på enheten. Av = alla lägen använder molnet.",
      enableLocalCleanup: "Aktivera lokal städning",
      enableLocalCleanupHelp:
        "När påslaget kör lägen med leverantör = Lokal (on-device LLM) städningen på enheten. När av faller dessa lägen tyst tillbaka på moln-workern.",
      sectionActiveMode: "Aktivt läge",
      addMode: "+ Nytt",
      noModes: "Inga lägen konfigurerade.",
      unsavedTitle: "Spara eller släng ändringar?",
      unsavedBody:
        "Du har osparade ändringar i det här läget. Spara eller släng dem innan du byter.",
      unsavedSave: "Spara",
      unsavedDiscard: "Kassera",
      downloadModelTitle: "Ladda ner modell?",
      downloadModelBody: (label, size) =>
        `Det här läget använder ${label}${size ? ` (${size})` : ""}, som inte är installerat än. Ladda ner nu?`,
      downloadModelDownload: "Ladda ner",
      downloadModelUseCloud: "Använd molnet tills vidare",
      dragHandleTitle: "Dra för att ändra ordning",
      duplicateTitle: "Duplicera det här läget",
      duplicateAriaLabel: (name) => `Duplicera ${name}`,
      newMode: "Nytt läge",
      newModeCopy: (name) => `Nytt läge — kopia av ${name}`,
      configureMode: (name) => `Konfigurera — ${name}`,
      autoModeAppsLabel: "App-Auto-läge",
      autoModeAppsHelp: (modeName) =>
        `När du växlar till en av dessa appar växlar inspelningen automatiskt till läget "${modeName}". Gäller bara när Konfiguration → Lägesval → Auto-läge är på.`,
      autoModeAppsEmpty: "Inga appar än — lägg till en för att automatiskt växla till detta läge.",
      autoModeAppsAdd: "+ Lägg till app…",
      autoModeAppsRemove: (appName) => `Ta bort ${appName}`,
      autoModeAppsPickerTitle: (modeName) => `Välj en app för läget ${modeName}`,
      editorStyle: "Stil",
      editorLanguage: "Språk",
      editorAutoCapitalize: "Automatisk versalisering",
      editorAutoCapitalizeHelp: "Versalisering på serversidan efter skiljetecken",
      editorModelsAccordion: "Modeller",
      editorTranscription: "Transkribering",
      editorTranscriptionHelp: "Vilken Whisper-variant som transkriberar i detta läge. Auto väljer baserat på språk.",
      editorProcessingProvider: "Bearbetningsleverantör",
      editorProcessingProviderHelp: "LLM-leverantör för cleanup",
      editorProcessingAi: "Bearbetnings-AI",
      editorPromptPlaceholder: "Beskriv hur AI:n ska städa upp din dikterade text…",
      editorSlugPlaceholder: "auto",
      editorSlugTitle: "Lägets slug — härleds automatiskt från namnet. Klicka här för att redigera. Låst efter sparande.",
      editorIdTitle: "Läges-ID — låst vid första sparande, ändras aldrig",
      editorIconPickerTitle: "Välj ikon",
      editorSave: "Spara",
      editorCancel: "Avbryt",
      editorDelete: "Ta bort",
      editorResetToDefault: "Återställ till standard",
      editorResetTooltip: "Du har avvikit från standard. Klicka på Återställ för att återgå.",
      editorPromptAccordion: "Prompt",
    },
    sound: {
      roundTripTest: "Tur-och-retur-test",
      roundTripTestHelp:
        "Spelar in 2s, skickar till workern och visar svaret. Testar pipelinen isolerat — utan snabbtangent, utan inklistring.",
      testRecordIdle: "Testinspelning",
      testRecordRecording: "Spelar in 2s…",
      testRecordTranscribing: "Skickar till worker…",
      compareIdle: "Spela in & jämför",
      compareRecording: "Spelar in 6s…",
      compareRunning: "Jämför…",
      compareRunAgain: "Kör igen",
      compareWhisperRaw: "Whisper (rå)",
      sectionMicrophone: "Mikrofon",
      sectionMicrophoneHelp:
        "Ultravox använder systemets standardmikrofon. Val per enhet kommer i v1.1.",
      sectionCompare: "Jämför städningskvalitet",
      sectionCompareHelp:
        "Spelar in 6s och kör sedan samma ljud parallellt genom Whisper rå + fyra LLM-varianter. Använd det här för att hitta rätt modell för din dikteringsstil.",
      sectionInputProcessing: "Indatabehandling",
      autoGain: "Autoförstärkning",
      autoGainHelp: "Webbläsaren justerar mikrofonnivån automatiskt",
      noiseSuppression: "Brusreducering",
      noiseSuppressionHelp:
        "Minskar bakgrundsbrus. Liten kvalitetsförlust.",
      silenceRemoval: "Ta bort tystnad",
      silenceRemovalHelp:
        "Klipp bort tysta partier före uppladdning (v1.1)",
      sectionSoundEffects: "Ljudeffekter",
      pauseMusic: "Pausa musik under inspelning",
      pauseMusicHelp:
        "Pausa Music och Spotify när en inspelning startar; återuppta när den stoppar.",
      duckMusic: "Sänk annat ljud under inspelning",
      duckMusicHelp:
        "Sänk Music/Spotify under inspelningen och återställ sedan. Mindre störande än att pausa om du vill fortsätta lyssna.",
      duckingDepth: "Sänkningsnivå",
      duckSubtle: "Mild",
      duckBalanced: "Balanserad",
      duckStrong: "Stark",
      duckTooltip: (pct) => `Sänk annat ljud med ${pct}% under inspelning`,
      chime: "Ton vid start/stopp",
      chimeHelp: "Kort ton när inspelningen startar och stoppar",
      chimeVolume: "Ton-volym",
      chimeTest: "Testa",
      chimeTestStart: "▶ Start",
      chimeTestStop: "▶ Stopp",
    },
    vocabulary: {
      title: "Ordlista",
      help: "Ersätt ord som Whisper konsekvent hör fel. Källan matchar utan att skilja på versaler/gemener; ersättningen behåller stavningen du skrev.",
      addEntry: "+ Lägg till post",
      sourceLabel: "Källa",
      replacementLabel: "Ersättning",
      empty: "Inga ordlistposter än.",
    },
    history: {
      sectionTitle: (count) => `Senaste transkriptioner (${count})`,
      clearAll: "Rensa alla",
      empty:
        "Inga transkriptioner än — gör en med inspelnings-snabbtangenten.",
      audioBadgeTitle: (size) => `Ljud sparat (${size})`,
      deleteAudio: "Radera ljud",
      footnoteAudio: (count) =>
        `${count} post${count === 1 ? "" : "er"} har sparat ljud.`,
      footnoteNoAudio:
        "Ljud sparas bara när 'Spara ljudinspelningar lokalt' är på i Konfiguration → Inspelningar.",
      footnoteCap: "De senaste 50 transkriptionerna bevaras.",
      timeJustNow: "just nu",
      timeMinutesAgo: (n) => `${n} min sedan`,
      timeHoursAgo: (n) => `${n} tim sedan`,
      timeDaysAgo: (n) => `${n} d sedan`,
      clickToCopy: "Klicka för att kopiera",
      expand: "Expandera",
      collapse: "Fäll ihop",
    },
  },
  pill: {
    discardConfirm: "Kassera?",
    discardKeepHint: "Mellanslag för att behålla",
    discardConfirmHint: "Esc för att kassera",
    transcribing: "Transkriberar…",
    silenceClosing: "Inget att transkribera. Stänger…",
    error: "Fel",
    dismiss: "Stäng",
    noSpeech:
      "Inget tal upptäckt. Gå närmare mikrofonen eller kontrollera din indataenhet.",
    nothingToTranscribe: "Inget att transkribera.",
    micDenied:
      "Mikrofonåtkomst nekad — aktivera i Systeminställningar → Integritet → Mikrofon.",
    micNotFound: "Ingen mikrofon hittades.",
    couldntStart: "Kunde inte starta inspelningen.",
    pasteFailed: (msg) =>
      `Inklistring misslyckades: ${msg} — hjälpmedelsåtkomst sannolikt nekad.`,
    invalidConstraint:
      "Ogiltig mikrofonkonfiguration — starta om Ultravox eller kontrollera Systeminställningar → Ljud → Indata.",
    silentDescription: "Inspelning pågår…",
    noAudioCaptured: "Inget ljud spelades in.",
    audioZeroBytes: "Inspelningen gav 0 byte — ljudformatet stöds inte.",
    transcribeFailed: (msg) => `Transkribering misslyckades: ${msg}`,
    openrouterKeyRequired:
      "OpenRouter-nyckel krävs. Inställningar → Konfiguration → Cleanup-leverantörer, eller byt detta läge till Claude Code.",
    discardRecording: "Kassera inspelning",
    doubleClickToExpand: "Dubbelklicka för att expandera",
  },
  modeOverlay: {
    title: "Byt läge",
    hint: "↑↓ för att välja · Enter för att bekräfta · Esc för att avbryta",
  },
  confirmDialog: {
    confirm: "Bekräfta",
    cancel: "Avbryt",
  },
  pillStylePicker: {
    classicLabel: "Klassisk",
    classicDescription: "Hel piller-form med vågform",
    miniLabel: "Mini",
    miniDescription: "Kompakta prickar längst upp på skärmen",
  },
  errors: {
    generic: "Något gick fel.",
  },
};

export const CATALOG: Record<Lang, MessageCatalog> = { en, de, es, sv };
